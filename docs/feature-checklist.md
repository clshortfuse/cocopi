...
- `[x]` Provider registration and per-model configuration target the VS Code 1.130 chat provider surface.
- `[x]` Mark Cocopi provider models as BYOK so VS Code treats them as user-credential-backed rather than Copilot/CAPI-backed.
- `[x]` Tag the package for VS Code 1.130+ Language Models editor provider discovery.
- `[x]` Expose reasoning effort and recommended/full context-size choices through VS Code's unified model customization picker.
- `[x]` Participate in VS Code 1.130 Agent Host Copilot SDK sessions through the host-owned BYOK language-model bridge; no Cocopi SDK dependency or runtime replacement is required.
...
- `[x]` Audit every product feature and community fix listed in the VS Code 1.130 release notes; assisted approvals, worktrees, timestamps, credit totals, terminal links, and Agents-window changes are host-owned or unrelated, with no Cocopi manifest/API migration.
- `[x]` Compare the exact VS Code `1.129.0` and `1.130.0` source/API trees; Cocopi's stable and enabled proposed declarations are unchanged, while BYOK snapshots and model/configuration restoration improve in the host.
- `[x]` Ensure a terminal `task_complete` replay emits concise text when the full answer is already visible; VS Code's Copilot BYOK adapter classifies responses with neither text nor tool calls as `unknown`, and a data-only response does not satisfy its success check.
