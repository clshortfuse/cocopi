...
VS Code 1.129 deprecates `languageModelChatProviders.managementCommand` in favor of the provider `configuration` schema. That replacement describes persisted provider groups (for example, an endpoint and API key), resolves secrets through VS Code, and forwards the resulting values to `provideLanguageModelChatInformation(options.configuration)`. Cocopi intentionally declares neither field: it is a singleton provider whose browser OAuth session remains in the extension's own SecretStorage flow, not a user-defined provider group. The separately contributed **Manage Cocopi** command and Chat status actions remain the setup and account-management surfaces without binding the deprecated provider-row gear action.

Current status as of 2026-06-18: Cocopi targets VS Code 1.125+ for the custom language model provider surface. The local `data/vscode-dts` proposal files include `LanguageModelChatProvider`, `LanguageModelChatInformation.configurationSchema`, `LanguageModelChatInformation.isUserSelectable`, `LanguageModelChatInformation.isBYOK`, `ProvideLanguageModelChatResponseOptions.modelConfiguration`, and `lm.registerLanguageModelChatProvider`. Cocopi implements the language model provider path against that 1.125 contract while keeping the `@cocopi` chat participant as a fallback/manual surface. Provider discovery exposes a generic configured model without requiring auth/network access, then refreshes model metadata from Codex `/models` at runtime when signed in. Cocopi exposes Codex catalog models as flat VS Code language model entries, marks them user-selectable and BYOK for the active chat picker, and adds one `:fast` picker variant when the catalog reports the `fast` speed tier and no catalog fast variant already exists. Cocopi advertises one per-model `configurationSchema` navigation property, `reasoningEffort`, titled `Thinking Effort`; its enum values and descriptions come from the flat Codex reasoning levels supported by that model. Its schema default and request fallback use the catalog default reasoning effort when present, falling back to the highest supported effort only when the catalog omits a default. Upstream Codex uses `supported_in_api` to filter API-key mode; ChatGPT mode keeps all catalog models. Cocopi uses ChatGPT authentication, so entries such as GPT-5.3-Codex-Spark remain routable and retain their advertised reasoning levels even when `supported_in_api: false`. For language-model provider requests, Cocopi asks for `summary: "auto"` when the catalog supports reasoning summaries but its default summary is `none`, so VS Code can receive native thinking summary events; explicit `cocopi.reasoningSummary: "off"` or request-specific summary overrides still win. It does not encode summary or speed-tier choices into comma-separated `reasoningEffort` values. Global `cocopi.reasoningEffort`, `cocopi.reasoningSummary`, `cocopi.serviceTier`, and request-specific `modelConfiguration`/`modelOptions` remain available for non-picker request construction.
...
Update 2026-06-18: Cocopi now targets VS Code 1.125+ for the chat provider proposal. Comparing `vscode.d.ts`, `vscode.proposed.chatProvider.d.ts`, and `vscode.proposed.languageModelThinkingPart.d.ts` from `1.122.0` through `1.125.0` found one relevant chat-provider API addition: `LanguageModelChatInformation.isBYOK`, added in `1.125.0`. Cocopi sets this for both signed-in catalog models and signed-out generic fallback metadata because requests use user-supplied ChatGPT/Codex credentials, not Copilot/CAPI. VS Code 1.125 uses this flag to classify selected models for quota surfaces and to skip Copilot fallback models when built-in subagents inherit a BYOK main model. VS Code 1.125 also added an **Install Model Providers** search in the Language Models editor that opens `tag:"language-models"`, so Cocopi includes the `language-models` keyword for published-build discoverability. The stable `vscode.d.ts` change in this range only reorders `CompletionItemKind.File` and `Reference`; `vscode.proposed.languageModelThinkingPart.d.ts` is unchanged.

Update 2026-06-25: Cocopi now targets VS Code 1.126+. The release's unified model customization picker combines `configurationSchema` properties grouped as `navigation` (thinking effort) and `tokens` (context size). Cocopi keeps `reasoningEffort` in `navigation` and emits a numeric `contextSize` property in `tokens` only when live catalog metadata advertises a meaningful choice, such as a lower `auto_compact_token_limit` or `max_context_window` greater than the default `context_window`. The schema default preserves the server-provided recommended window while `maxInputTokens` advertises the full usable window so users can opt into longer sessions from the picker.

Update 2026-07-15: Cocopi now targets VS Code 1.129+. The release introduces a dedicated Agent Host process and Agent Host Protocol, with the built-in Copilot harness powered by `@github/copilot-sdk`. That package is an application SDK around the Copilot CLI runtime, not a new public VS Code extension API, so Cocopi does not add it as a dependency or replace its direct ChatGPT OAuth/Codex backend path. VS Code's Agent Host BYOK bridge instead enumerates existing renderer language models whose metadata has `isBYOK: true` and no `targetChatSessionType`, then routes the selected vendor/model back through `ILanguageModelsService.sendChatRequest`. Cocopi already meets that contract with user-selectable, tool-capable BYOK metadata. The bridge is controlled by `chat.agentHost.byokModels.enabled` (default `true`, restart required) and remains subordinate to Agent Host enablement. Its current translation carries text messages, tool definitions, assistant tool calls, and tool results, but buffers output and ignores provider thinking/data response parts; Cocopi's streamed reasoning and `stateful_marker` continuation metadata therefore remain available only on the normal language-model provider path. The 1.129 declaration refresh is otherwise additive: `LanguageModelChatInformation` gains optional `warningText` and `promo` fields, neither of which Cocopi currently needs.

Exact-source supplement 2026-07-15: an uncapped local comparison of the exact VS Code `1.128.1` and `1.129.0` trees was used to audit changes that were not individually described in the release notes. Commit subjects were used only for triage because the release histories diverge and some commits in the range produce no final tag-tree delta. The Agent Host BYOK bridge changed from disabled to enabled by default and now round-trips the renderer model's identifier into `byokModelIdentifier`. That identity lets the Agent Host picker and **Manage Models** visibility state follow the original Cocopi entry instead of treating a synthetic Agent Host copy as a second independent model. The synthetic entry still does not copy the provider's `configurationSchema`. Its OpenAI Chat Completions proxy translates only `temperature`, `top_p`, and `max_tokens` into provider `modelOptions`; it does not forward `reasoning_effort`. Cocopi already accepts direct, persisted, snake-case, and nested reasoning option shapes, so this is a host bridge limitation rather than a Cocopi parser gap.

The same exact-tree audit found host-side fixes that benefit Cocopi without extension changes: edited requests retain the selected model's scoped configuration when the model is unchanged; Agent Host warns before pasting text that would execute as a leading `!` command; and MCP authentication now preserves empty granted scopes, authorization-server/resource context, account identity, and per-conversation prompt state. VS Code continues to own MCP credentials, consent, and tool execution. Other candidate changes were intentionally not adopted: Messages-API reasoning-effort forwarding belongs to the built-in Copilot custom-endpoint adapter, the windowed-token-cache eviction fix belongs to Copilot inline completions, and model promotion metadata should remain unset without authenticated Cocopi backend data. No additional language-model-provider manifest/schema migration was present beyond the deprecated `managementCommand` removal already handled.

The changed proposal watchlist is also additive or unrelated to Cocopi's enabled surface. `chatContextProvider` splits explicit attachments from tab-aware context and leaves deprecated compatibility shims; `agentEditorComments` adds comment-acceptance state and deletion; `chatSessionsProvider` gains promotion metadata equivalent to `chatProvider`. Cocopi enables none of those proposals and should not adopt them without a concrete provider-scoped need.

Update 2026-07-22: Cocopi now targets VS Code 1.130+. The stable `vscode.d.ts` file and Cocopi's enabled `chatProvider`, `chatStatusItem`, and `languageModelThinkingPart` proposal declarations are byte-for-byte unchanged from 1.129. The only changed proposed declaration is the unused `chatContextProvider`, which replaces a context item's `icon` field with `iconPath`; Cocopi does not enable that proposal. The exact `1.129.0` to `1.130.0` source comparison found no Cocopi declaration or manifest migration. Agent Host now receives pushed, change-driven BYOK model snapshots from eligible renderer windows instead of pulling model lists on demand, caches the serving window's current catalog, and avoids allowing an empty or non-serving window to mask an available provider. Cocopi's existing `onDidChangeLanguageModelChatInformation` event supplies the host with catalog/auth changes through the ordinary language-model service.

VS Code 1.130 also consolidates model selection across the workbench and Agents window, waits for asynchronous vendor resolution before treating a selected model as unavailable, migrates remembered selection state to profile scope, and hardens per-editor model-configuration restore against delayed schemas and stale values. These host fixes benefit Cocopi's live catalog and `reasoningEffort`/`contextSize` configuration without changing the provider contract. Agent Host assisted permissions, risk explanations, worktree isolation across Copilot/Claude/Codex harnesses, chat timestamps, and native Codex customizations remain host-owned. Aggregate Copilot Business/Enterprise AI credits and quota-trajectory fixes remain global Copilot-account state and must not be reused for Cocopi quotas. The BYOK bridge is still lossy: it buffers output and omits Cocopi thinking and hidden `LanguageModelDataPart` markers, so ordinary VS Code Chat remains the full-fidelity path. VS Code 1.130 still uses Electron 42.6.0 with Node.js 24.18.0; the upstream TypeScript 7 compiler/extension migration does not require Cocopi to rewrite its plain-JavaScript runtime or adopt the Copilot SDK.

Follow-up 2026-07-24: real-host validation exposed a terminal replay compatibility gap after `task_complete`. When the complete answer was already visible, Cocopi returned from the replay request without reporting a response part. VS Code's Copilot BYOK adapter classifies a request as successful only when it receives nonempty text or at least one tool call; a data-only response is consumed for metadata but still becomes `unknown`. Cocopi now emits the concise text `Task completed.` on an already-visible terminal replay, preserving the full answer without repeating it while giving the host a recognized successful response. A model follow-up remains reserved for completion calls with neither a summary nor prior visible assistant text.

Update 2026-06-19: VS Code autocomplete is separate from the chat language model provider surface. Classic IntelliSense flows through `CompletionItemProvider`/LSP `textDocument/completion`; ghost-text AI autocomplete flows through `InlineCompletionItemProvider`. VS Code orders providers through `LanguageFeatureRegistry`, runs classic completion provider groups by score, and asks inline completion providers through `provideInlineCompletions(...)`. The 1.122→1.125 source audit found LSP 3.18 completion/inline-completion protocol work and a proposed `inlineCompletionsAdditions` API for inline provider metadata/model options, but Cocopi does not need that proposal for a first implementation. Cocopi now registers a stable `InlineCompletionItemProvider` and keeps model selection in normal extension settings/commands: `cocopi.inlineCompletions.enabled`, `cocopi.inlineCompletions.model`, bounded prefix/suffix context settings, **Cocopi: Toggle Inline Completions**, and **Cocopi: Set Inline Completion Model**. The default inline model mode is `auto`, which asks the signed-in Codex model catalog for a Spark-like low-latency model available to the account, falls back to GPT-5.6 Luna, and uses `cocopi.model` only after those specialized targets; explicit available model ids bypass automatic selection. Inline completion requests use the same SecretStorage auth-refresh and Responses streaming stack as chat requests, with no tool calls and inline-specific cancellation/idle timeout behavior. For manual testing, `cocopi.debugLevel: events` logs inline request metadata and Responses event types, while `payloads` also logs request/event payloads and can include surrounding editor text.
