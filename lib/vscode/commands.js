...
async function showCocopiStatusWindow(context, vscode) {
  ...
  const render = async () => {
    ...
    panel.webview.html = cocopiStatusWindowHtml(snapshot.auth, snapshot.runtime, snapshot.usage, snapshot.issueCount, snapshot.models);
  };
  await render();
 ...
 */
function cocopiStatusWindowHtml(auth, runtime, usage, issueCount, models = []) {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .status-card, .control-card, .usage-panel, .links { background: var(--vscode-editorWidget-background); border: 1px solid var(--vscode-editorWidget-border); border-radius: 14px; box-shadow: 0 8px 24px rgb(0 0 0 / 0.14); }
...
<body>
  <main aria-label="Cocopi status">
    ...
    <p class="hint">Edit common Cocopi settings here; use Token Tracker and Diagnostics for detailed history.</p>
  </main>
  <script>
...
<head>
  ...
  <style>
    ...
    .chart-grid { display: grid; gap: 10px; grid-template-columns: repeat(auto-fit, minmax(720px, 1fr)); }
    ...
    .timeline-chart { background: color-mix(in srgb, var(--vscode-editor-background) 92%, var(--vscode-editorWidget-border)); border: 1px solid var(--vscode-editorWidget-border); border-radius: 4px; display: block; height: auto; min-width: 680px; width: 100%; }
    ...
    .analytics-table { border-collapse: collapse; table-layout: fixed; width: max(100%, var(--analytics-table-min-width, 72ch)); }
    ...
    .analytics-table th:first-child, .analytics-table td:first-child { min-width: var(--analytics-first-column-width, 14ch); text-align: left; width: var(--analytics-first-column-width, 14ch); }
    .analytics-table th:not(:first-child), .analytics-table td:not(:first-child) { min-width: var(--analytics-column-width, 9ch); width: var(--analytics-column-width, 9ch); }
