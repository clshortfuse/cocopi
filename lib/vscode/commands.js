...
import { COCOPI_CONFIGURATION_SECTION, COCOPI_INLINE_COMPLETION_MODEL_AUTO, COCOPI_TOKEN_TRACKER_TIMELINE_MODES, DEFAULT_COCOPI_CHAT_INSTRUCTIONS_REGEX_REPLACEMENTS, DEFAULT_COCOPI_CHAT_TOOL_DESCRIPTION_REGEX_REPLACEMENTS } from "./configuration.js";
import { noopCocopiLogger, readCocopiMemoryUsage } from "./diagnostics.js";
import { onCocopiInstructionReplacementSnapshot, readCocopiInstructionReplacementSnapshots } from "./instruction-replacements.js";
 ...
 * @property {(key: string, value: string | number | boolean | Record<string, string>, target?: boolean) => Thenable<void>} [update]
 */
...
export function registerCocopiCommands(context, vscode, options = {}) {
  ...
  statusSurfaces = registerCocopiStatusSurfaces(context, vscode, options.logger ?? noopCocopiLogger);
}
 ...
 */
function registerCocopiStatusSurfaces(context, vscode, logger) {
  const controllers = [
    registerCocopiStatusBar(context, vscode),
    registerCocopiChatStatusItem(context, vscode, logger)
 ...
 * @returns {CocopiStatusSurfaceController | undefined}
 */
function registerCocopiChatStatusItem(context, vscode, logger) {
  ...
  let item;
  try {
    item = vscode.window.createChatStatusItem("cocopi.status");
  } catch (error) {
    logger.info(`Cocopi native Chat status item is unavailable; using stable status surfaces. error=${error instanceof Error ? error.message : String(error)}`);
    return;
  }
  item.title = "Cocopi";
...
async function showCocopiStatusWindow(context, vscode) {
  ...
  const render = async () => {
    ...
    panel.webview.html = cocopiStatusWindowHtml(snapshot.auth, snapshot.runtime, snapshot.usage, snapshot.issueCount, snapshot.featureAudit, snapshot.models, snapshot.utilityRouting, snapshot.instructionReplacements);
  };
  await render();
  const unsubscribeInstructionReplacements = onCocopiInstructionReplacementSnapshot((scope, snapshot) => {
    void panel.webview.postMessage?.({
      type: "instructionReplacementSnapshot",
      scope,
      snapshot,
      textsHtml: cocopiStatusInstructionReplacementTextsHtml(snapshot, scope === "instructions" ? "Chat instructions" : "Tool description")
    });
  });
  panel.onDidDispose?.(unsubscribeInstructionReplacements);
  ...
  if (typeof panel.webview.onDidReceiveMessage === "function") {
    panel.webview.onDidReceiveMessage(async (message) => {
      ...
      if (record.type === "updateInstructionReplacements") {
        await updateCocopiStatusPanelInstructionReplacements(vscode, record);
        return;
      }
...
async function readCocopiStatusSnapshot(context, vscode) {
  ...
  return {
    ...
    instructionReplacements: readCocopiInstructionReplacementSnapshots(),
...
function readCocopiStatusUtilityRouting(vscode, runtime, models) {
  ...
  const recommended = mode === "specific"
    && utilityModelId === "utility"
    && utilitySmallModelId === "utility-small";
  return {
    available,
    mode,
 ...
 */
async function updateCocopiStatusPanelInstructionReplacements(vscode, message) {
  const setting = message.scope === "instructions"
    ? "chatInstructionsRegexReplacements"
    : (message.scope === "toolDescriptions" ? "chatToolDescriptionRegexReplacements" : undefined);
  if (!setting || !message.replacements || typeof message.replacements !== "object" || Array.isArray(message.replacements)) {
    return;
  }

  /** @type {Record<string, string>} */
  const replacements = {};
  for (const [pattern, replacement] of Object.entries(message.replacements).slice(0, 100)) {
    if (pattern.trim() && pattern.length <= 10_000 && typeof replacement === "string" && replacement.length <= 100_000) {
      replacements[pattern] = replacement;
    }
  }

  const configuration = /** @type {ModelConfigurationUpdate} */ (vscode.workspace.getConfiguration(COCOPI_CONFIGURATION_SECTION));
  if (typeof message.flags === "string" && validRegexFlags(message.flags)) {
    await configuration.update?.("chatRegexFlags", message.flags, true);
  }
  await configuration.update?.(setting, replacements, true);
}

/** @param {string} flags */
function validRegexFlags(flags) {
  try {
...
async function updateCocopiStatusPanelUtilityRouting(vscode, message) {
  ...
  if (mode === "recommended") {
    await configuration.update?.("utilityModel", "cocopi/utility", true);
    await configuration.update?.("utilitySmallModel", "cocopi/utility-small", true);
...
function statusPanelInlineCompletionChoiceValues(value) {
  ...
  if (text === COCOPI_INLINE_COMPLETION_MODEL_AUTO) {
    return [
      ...
      { key: "routes.autocomplete.model", value: COCOPI_INLINE_COMPLETION_MODEL_AUTO }
    ];
  }
  ...
  if (text.startsWith("model:")) {
    ...
    return model ? [
      ...
      { key: "routes.autocomplete.model", value: model }
    ] : [];
  }
...
function statusPanelConfigurationValue(key, value) {
  if (/^routes\.(?:utility|utilitySmall|autocomplete)\.model$/u.test(key)) {
    const text = typeof value === "string" ? value.trim() : "";
    if (text.endsWith(":fast") || isCocopiWorkloadAlias(text)) {
      return;
    }
    return { key, value: text };
  }

  if (/^routes\.(?:utility|utilitySmall|autocomplete)\.reasoningEffort$/u.test(key) && typeof value === "string" && ["lowest", "default", "none", "minimal", "low", "medium", "high", "xhigh", "max"].includes(value)) {
    return { key, value };
  }

  if (/^routes\.(?:utility|utilitySmall|autocomplete)\.serviceTier$/u.test(key) && typeof value === "string" && ["auto", "flex", "priority"].includes(value)) {
    return { key, value };
  }
...
/**
 * @param {{ id: string, title: string, summary: string, content: string, open?: boolean }} options
 */
function cocopiStatusDisclosureHtml(options) {
  if (!options.content) {
    return "";
  }
  return `
    <details class="dashboard-section" data-dashboard-disclosure="${escapeHtml(options.id)}"${options.open ? " open" : ""}>
      <summary class="dashboard-section-summary">
        <span class="dashboard-section-title">${escapeHtml(options.title)}</span>
        <span class="dashboard-section-status">${escapeHtml(options.summary)}</span>
      </summary>
      <div class="dashboard-section-body">${options.content}</div>
    </details>`;
}

/** @param {StatusPanelUtilityRouting} routing */
function cocopiStatusUtilityRoutingBrief(routing) {
  if (routing.state === "recommended") {
    return "Recommended utility routes";
  }
  switch (routing.mode) {
    case "none": {
      return "Background tasks off";
    }
    case "mainAgent": {
      return "Background uses chat model";
    }
    case "copilot": {
      return "Background uses GitHub Copilot";
    }
    case "specific": {
      return routing.state === "required" ? "Utility setup required" : "Custom utility models";
    }
    default: {
      return "External utility provider";
    }
  }
}
 ...
 * @param {ReturnType<typeof readCocopiInstructionReplacementSnapshots>} [instructionReplacements]
 */
function cocopiStatusWindowHtml(auth, runtime, usage, issueCount, featureAudit, models = [], utilityRouting, instructionReplacements = {}) {
  const actions = cocopiStatusDashboardActionsHtml(auth);
  const features = cocopiStatusFeatureAuditHtml(featureAudit);
  const autocomplete = cocopiStatusAutocompleteHtml(runtime, models, featureAudit);
  const chatRouting = cocopiStatusChatRoutingHtml(runtime, models, utilityRouting);
  const diagnostics = cocopiStatusDiagnosticsSettingsHtml(runtime);
  const replacementPreview = cocopiStatusInstructionReplacementsHtml(runtime, instructionReplacements);
  const usageMeters = cocopiStatusUsageMetersHtml(usage);
  const autocompleteRoute = resolveCocopiWorkloadRoute(COCOPI_WORKLOAD_ROUTE_NAMES.autocomplete, runtime.configuration, models);
  const unhealthyFeatures = featureAudit.filter((item) => item.state === "limited" || item.state === "disabled").length;
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    main { display: grid; gap: 12px; margin: 0 auto; max-width: 940px; }
    ...
    .status-card, .control-card, .feature-panel, .feature-card, .usage-panel, .replacement-panel, .dashboard-section, .links { background: var(--vscode-editorWidget-background); border: 1px solid var(--vscode-editorWidget-border); border-radius: 14px; box-shadow: 0 8px 24px rgb(0 0 0 / 0.14); }
    ...
    .dashboard-section[open] { box-shadow: 0 10px 28px rgb(0 0 0 / 0.18); }
    .dashboard-section-summary { align-items: center; cursor: pointer; display: flex; gap: 12px; justify-content: space-between; list-style: none; padding: 13px 14px; }
    .dashboard-section-summary::-webkit-details-marker { display: none; }
    .dashboard-section-summary::before { color: var(--vscode-descriptionForeground); content: '›'; font-size: 20px; line-height: 1; transform: rotate(0deg); transition: transform 120ms ease; }
    .dashboard-section[open] > .dashboard-section-summary::before { transform: rotate(90deg); }
    .dashboard-section-title { flex: 1 1 auto; font-size: 14px; font-weight: 700; }
    .dashboard-section-status { color: var(--vscode-descriptionForeground); font-size: 12px; text-align: right; }
    .dashboard-section-body { border-top: 1px solid var(--vscode-editorWidget-border); display: grid; gap: 12px; padding: 14px; }
    .dashboard-section-body > .control-card, .dashboard-section-body > .feature-panel, .dashboard-section-body > .usage-panel, .dashboard-section-body > .replacement-panel { box-shadow: none; }
    .section-intro { display: grid; gap: 6px; }
    .section-intro h2, .section-intro p { margin: 0; }
    .state-strip { display: grid; gap: 8px; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); }
    .state-item { border-left: 3px solid var(--vscode-textLink-foreground); display: grid; gap: 2px; padding: 3px 8px; }
    .state-label { color: var(--vscode-descriptionForeground); font-size: 11px; }
    .nested-disclosure { border: 1px solid var(--vscode-editorWidget-border); border-radius: 9px; }
    .nested-disclosure > summary { cursor: pointer; font-weight: 650; padding: 10px; }
    .nested-disclosure-body { border-top: 1px solid var(--vscode-editorWidget-border); display: grid; gap: 12px; padding: 10px; }
    .workload-route + .workload-route { border-top: 1px dashed var(--vscode-editorWidget-border); padding-top: 12px; }
    .workload-route .field-grid { grid-template-columns: repeat(auto-fit, minmax(min(100%, 32ch), 1fr)); }
    .workload-route .field-grid > label { min-width: 0; }
    .workload-route select { max-width: 100%; min-width: 0; }
    ...
    .utility-state.off { background: color-mix(in srgb, var(--vscode-descriptionForeground) 16%, transparent); color: var(--vscode-descriptionForeground); }
    ...
    .replacement-panel { display: grid; gap: 12px; padding: 14px; }
    .replacement-panel > div > p { margin: 0; }
    .replacement-scope { border: 1px solid var(--vscode-editorWidget-border); border-radius: 10px; padding: 10px; }
    .replacement-scope summary { cursor: pointer; font-weight: 650; }
    .replacement-workbench { display: grid; gap: 10px; margin-top: 10px; }
    .replacement-texts { display: grid; gap: 10px; }
    .replacement-text { display: grid; gap: 6px; }
    .replacement-text-head { align-items: center; display: flex; gap: 8px; }
    .replacement-text-head h3 { margin: 0; }
    .replacement-text-spacer { flex: 1 1 auto; }
    .replacement-text-status { border-radius: 999px; font-size: 11px; font-weight: 650; padding: 2px 8px; }
    .replacement-text-status[hidden] { display: none; }
    .replacement-text-matched { background: color-mix(in srgb, var(--vscode-testing-iconPassed, #73c991) 22%, transparent); color: var(--vscode-testing-iconPassed, #73c991); }
    .replacement-text-modified { background: color-mix(in srgb, var(--vscode-textLink-foreground) 18%, transparent); color: var(--vscode-textLink-foreground); }
    .replacement-columns { display: grid; gap: 10px; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); }
    textarea { background: var(--vscode-input-background); border: 1px solid var(--vscode-input-border, var(--vscode-editorWidget-border)); border-radius: 6px; color: var(--vscode-input-foreground); font-family: var(--vscode-editor-font-family); font-size: 12px; line-height: 1.35; min-height: 120px; padding: 8px; resize: vertical; width: 100%; }
    textarea[readonly] { background: var(--vscode-textCodeBlock-background); }
    .replacement-rules { display: grid; gap: 8px; }
    .replacement-rule { background: color-mix(in srgb, var(--vscode-editor-background) 65%, transparent); border: 1px solid color-mix(in srgb, var(--vscode-editorWidget-border) 80%, transparent); border-radius: 8px; display: grid; gap: 7px; padding: 9px; }
    .replacement-rule-head { align-items: center; display: flex; gap: 8px; }
    .replacement-rule-spacer { flex: 1 1 auto; }
    .replacement-match { background: color-mix(in srgb, var(--vscode-testing-iconPassed, #73c991) 22%, transparent); border-radius: 999px; color: var(--vscode-testing-iconPassed, #73c991); font-size: 11px; font-weight: 650; padding: 2px 8px; }
    .replacement-match[hidden] { display: none; }
    .replacement-rule textarea { min-height: 64px; }
    .replacement-rule-pattern { min-height: 48px !important; }
    .replacement-actions { align-items: center; display: flex; flex-wrap: wrap; gap: 8px; }
    .replacement-result { color: var(--vscode-descriptionForeground); font-size: 12px; }
    .replacement-result.error { color: var(--vscode-editorError-foreground, #f14c4c); }
    ...
    .links { align-items: center; box-shadow: none; display: flex; flex-wrap: wrap; gap: 8px; padding: 10px 12px; }
  </style>
</head>
<body>
  <main aria-label="Cocopi status">
    ...
    ${usageMeters}
    ${cocopiStatusDisclosureHtml({
      id: "autocomplete",
      title: "Autocomplete",
      summary: runtime.configuration.inlineCompletions.enabled ? `On · ${autocompleteRoute.targetModel}` : "Off",
      content: autocomplete,
      open: true
    })}
    ${cocopiStatusDisclosureHtml({
      id: "chat-routing",
      title: "Chat and background tasks",
      summary: utilityRouting ? `${runtime.configuration.model} · ${cocopiStatusUtilityRoutingBrief(utilityRouting)}` : runtime.configuration.model,
      content: chatRouting
    })}
    ${cocopiStatusDisclosureHtml({
      id: "health",
      title: "Health and feature gates",
      summary: unhealthyFeatures === 0 ? "All enabled" : `${unhealthyFeatures} limited or off`,
      content: features
    })}
    ${cocopiStatusDisclosureHtml({
      id: "advanced",
      title: "Advanced",
      summary: "Diagnostics and instruction transforms",
      content: `${diagnostics}${replacementPreview}`
    })}
  </main>
  <script>
    const vscode = acquireVsCodeApi();
    const dashboardState = vscode.getState?.() ?? {};
    const rememberedDisclosures = dashboardState.disclosures ?? {};
    const persistDashboardState = (changes) => {
      vscode.setState?.({ ...(vscode.getState?.() ?? dashboardState), ...changes });
    };
    for (const disclosure of document.querySelectorAll('[data-dashboard-disclosure]')) {
      const disclosureId = disclosure.dataset.dashboardDisclosure;
      if (disclosureId && Object.hasOwn(rememberedDisclosures, disclosureId)) {
        disclosure.open = Boolean(rememberedDisclosures[disclosureId]);
      }
      disclosure.addEventListener('toggle', () => {
        const disclosures = { ...(vscode.getState?.()?.disclosures ?? rememberedDisclosures) };
        if (disclosureId) {
          disclosures[disclosureId] = disclosure.open;
        }
        persistDashboardState({ disclosures });
      });
    }
    if (Number.isFinite(dashboardState.scrollY)) {
      requestAnimationFrame(() => scrollTo(0, dashboardState.scrollY));
    }
    let scrollStateFrame;
    window.addEventListener('scroll', () => {
      cancelAnimationFrame(scrollStateFrame);
      scrollStateFrame = requestAnimationFrame(() => persistDashboardState({ scrollY }));
    }, { passive: true });
    document.addEventListener('click', (event) => {
      ...
      const addReplacement = event.target?.closest?.('[data-add-replacement]');
      if (addReplacement) {
        event.preventDefault();
        addInstructionReplacementRule(addReplacement.closest('[data-replacement-scope]'));
        return;
      }

      const removeReplacement = event.target?.closest?.('[data-remove-replacement]');
      if (removeReplacement) {
        event.preventDefault();
        const scope = removeReplacement.closest('[data-replacement-scope]');
        removeReplacement.closest('.replacement-rule')?.remove();
        updateInstructionReplacementPreview(scope);
        return;
      }

      const applyReplacements = event.target?.closest?.('[data-apply-replacements]');
      if (applyReplacements) {
        event.preventDefault();
        applyInstructionReplacements(applyReplacements.closest('[data-replacement-scope]'));
        return;
      }
    ...
    document.addEventListener('input', (event) => {
      if (event.target?.id === 'instructionReplacementFlags') {
        for (const scope of document.querySelectorAll('[data-replacement-scope]')) {
          updateInstructionReplacementPreview(scope);
        }
        return;
      }
      updateInstructionReplacementPreview(event.target?.closest?.('[data-replacement-scope]'));
    });
    ...
    window.addEventListener('message', (event) => {
      ...
      if (message?.type !== 'instructionReplacementSnapshot'
        || (message.scope !== 'instructions' && message.scope !== 'toolDescriptions')
        || typeof message.snapshot?.original !== 'string') {
        return;
      }

      const scope = document.querySelector('[data-replacement-scope="' + message.scope + '"]');
      const texts = scope?.querySelector('[data-replacement-texts]');
      const capture = scope?.querySelector('[data-replacement-capture]');
      if (!scope || !texts || typeof message.textsHtml !== 'string') {
        return;
      }

      texts.innerHTML = message.textsHtml;
      if (capture) {
        const capturedAt = new Date(message.snapshot.capturedAt);
        const capturedLabel = Number.isNaN(capturedAt.getTime()) ? 'just now' : capturedAt.toLocaleString();
        capture.textContent = 'Last captured ' + capturedLabel + (message.snapshot.truncated ? ' · preview truncated' : '');
      }
      updateInstructionReplacementPreview(scope);
    });

    function replacementRules(scope) {
      return [...scope.querySelectorAll('.replacement-rule')].map((rule) => ({
        enabled: rule.querySelector('[data-replacement-enabled]')?.checked ?? true,
        pattern: rule.querySelector('[data-replacement-pattern]')?.value ?? '',
        replacement: rule.querySelector('[data-replacement-value]')?.value ?? '',
        matchBadge: rule.querySelector('[data-replacement-match]')
      }));
    }

    function updateInstructionReplacementPreview(scope) {
      if (!scope) {
        return true;
      }

      const result = scope.querySelector('[data-replacement-result]');
      const flags = document.getElementById('instructionReplacementFlags')?.value ?? 'g';
      const errors = [];
      const patterns = new Set();
      const activeRules = [];
      for (const rule of replacementRules(scope)) {
        if (rule.matchBadge) {
          rule.matchBadge.hidden = true;
        }
      ...
      const pairs = [...scope.querySelectorAll('[data-replacement-text]')];
      for (const pair of pairs) {
        const sourceText = pair.querySelector('[data-replacement-source]')?.value ?? '';
        const preview = pair.querySelector('[data-replacement-preview]');
        const matchedBadge = pair.querySelector('[data-replacement-text-matched]');
        const modifiedBadge = pair.querySelector('[data-replacement-text-modified]');
        const label = pair.dataset.replacementLabel ?? 'Text';
        let text = sourceText;
        let matched = false;
        for (const rule of activeRules) {
          if (new RegExp(rule.pattern, flags).test(sourceText)) {
            const labels = matchedRules.get(rule) ?? new Set();
            labels.add(label);
            matchedRules.set(rule, labels);
            matched = true;
          }
          text = text.replace(new RegExp(rule.pattern, flags), rule.replacement);
        }
        if (preview) {
          preview.value = text;
        }
        if (matchedBadge) {
          matchedBadge.hidden = !matched;
        }
        if (modifiedBadge) {
          modifiedBadge.hidden = text === sourceText;
        }
        pair.dataset.replacementModified = String(text !== sourceText);
      }
      pairs.sort((left, right) => {
        const modificationOrder = Number(right.dataset.replacementModified === 'true') - Number(left.dataset.replacementModified === 'true');
        return modificationOrder || (left.dataset.replacementLabel ?? '').localeCompare(right.dataset.replacementLabel ?? '', undefined, { sensitivity: 'base' });
      });
      scope.querySelector('[data-replacement-texts]')?.append(...pairs);
      for (const rule of activeRules) {
        if (rule.matchBadge) {
          const labels = [...(matchedRules.get(rule) ?? [])].toSorted((left, right) => left.localeCompare(right));
          rule.matchBadge.hidden = labels.length === 0;
          rule.matchBadge.textContent = labels.length > 0 ? 'Matched: ' + labels.join(', ') : 'Matched';
        }
      }
      if (result) {
        result.textContent = errors.length > 0 ? errors.join(' · ') : '';
        result.classList.toggle('error', errors.length > 0);
      }
      return errors.length === 0;
    }

    function addInstructionReplacementRule(scope) {
      const rules = scope?.querySelector('.replacement-rules');
      if (!scope || !rules) {
        return;
      }
      const rule = document.createElement('article');
      rule.className = 'replacement-rule';
      rule.innerHTML = '<div class="replacement-rule-head"><span class="field-hint">Custom</span><span class="replacement-rule-spacer"></span><span class="replacement-match" data-replacement-match hidden>Matched</span><button type="button" data-remove-replacement>Remove</button></div><label>Regex pattern<textarea class="replacement-rule-pattern" data-replacement-pattern spellcheck="false"></textarea></label><label>Replacement text<textarea data-replacement-value placeholder="(Remove matched text)" spellcheck="false"></textarea></label>';
      rules.prepend(rule);
      rule.querySelector('[data-replacement-pattern]')?.focus();
      updateInstructionReplacementPreview(scope);
    }

    function applyInstructionReplacements(scope) {
      if (!scope || !updateInstructionReplacementPreview(scope)) {
        return;
      }
      const replacements = {};
      for (const rule of replacementRules(scope)) {
        if (rule.pattern.trim()) {
          replacements[rule.pattern] = rule.enabled ? rule.replacement : '';
        }
      }
      vscode.postMessage({
        type: 'updateInstructionReplacements',
        scope: scope.dataset.replacementScope,
        flags: document.getElementById('instructionReplacementFlags')?.value ?? 'g',
        replacements
      });
      const result = scope.querySelector('[data-replacement-result]');
      if (result) {
        result.textContent = 'Rules applied to Cocopi user settings.';
      }
    }
    ...
    function updateUtilitySetupPreview() {
      ...
      const descriptions = {
        recommended: "Use Cocopi's specialized general and small utility workload models.",
      ...
      const settings = {
        'chat.utilityModel': choice === 'recommended' ? 'cocopi/utility' : (choice === 'specific' ? utilityModel?.value ?? '' : ''),
        'chat.utilitySmallModel': choice === 'recommended' ? 'cocopi/utility-small' : (choice === 'specific' ? utilitySmallModel?.value ?? '' : ''),
...
/**
 * @param {import("./runtime.js").CocopiRuntime} runtime
 * @param {ReturnType<typeof readCocopiInstructionReplacementSnapshots>} snapshots
 */
function cocopiStatusInstructionReplacementsHtml(runtime, snapshots) {
  return `
    <section class="replacement-panel" aria-label="Instruction replacements">
      <div>
        <h2>Instruction replacements</h2>
        <p>Inspect the last host text Cocopi saw, toggle or edit regex rules, and preview the exact replacement locally before applying it.</p>
        <p class="field-hint">Built-in rules have an Enabled checkbox. Custom rules are active while present; leave Replacement text blank to remove matching text.</p>
        <p class="field-hint">Captured text stays only in extension-host memory and is cleared when the extension host restarts.</p>
      </div>
      <label>
        Regex flags
        <input id="instructionReplacementFlags" type="text" value="${escapeHtml(runtime.configuration.chatRegexFlags)}" spellcheck="false">
      </label>
      ${cocopiStatusInstructionReplacementScopeHtml({
        scope: "instructions",
        title: "Chat instructions",
        snapshot: snapshots.instructions,
        replacements: runtime.configuration.chatInstructionsRegexReplacements,
        defaults: DEFAULT_COCOPI_CHAT_INSTRUCTIONS_REGEX_REPLACEMENTS,
        open: true
      })}
      ${cocopiStatusInstructionReplacementScopeHtml({
        scope: "toolDescriptions",
        title: "Tool descriptions",
        snapshot: snapshots.toolDescriptions,
        replacements: runtime.configuration.chatToolDescriptionRegexReplacements,
        defaults: DEFAULT_COCOPI_CHAT_TOOL_DESCRIPTION_REGEX_REPLACEMENTS,
        open: false
      })}
    </section>`;
}
...
/**
 * @param {{ scope: "instructions" | "toolDescriptions", title: string, snapshot?: import("./instruction-replacements.js").CocopiInstructionReplacementSnapshot, replacements: Readonly<Record<string, string>>, defaults: Readonly<Record<string, string>>, open: boolean }} options
 */
function cocopiStatusInstructionReplacementScopeHtml(options) {
  const snapshot = options.snapshot;
  const captureDetail = snapshot
    ? `Last captured ${formatTokenCacheTimestamp(snapshot.capturedAt)}${snapshot.truncated ? " · preview truncated" : ""}`
    : "No host text captured yet — run a Cocopi chat request or paste sample text below.";
  return `
      <details class="replacement-scope" data-replacement-scope="${options.scope}"${options.open ? " open" : ""}>
        <summary>${escapeHtml(options.title)}</summary>
        <div class="replacement-workbench">
          <p class="field-hint" data-replacement-capture>${escapeHtml(captureDetail)}</p>
          <div class="replacement-rules">
            ${cocopiStatusInstructionReplacementRulesHtml(options.replacements, options.defaults)}
          </div>
          <div class="replacement-texts" data-replacement-texts>${cocopiStatusInstructionReplacementTextsHtml(snapshot, options.title)}</div>
          <div class="replacement-actions">
            <button type="button" data-add-replacement>Add rule</button>
            <button class="primary" type="button" data-apply-replacements>Apply rules</button>
            <span class="replacement-result" data-replacement-result aria-live="polite"></span>
          </div>
        </div>
      </details>`;
}
 ...
 * @param {Readonly<Record<string, string>>} defaults
 */
function cocopiStatusInstructionReplacementRulesHtml(replacements, defaults) {
  return Object.entries(replacements)
    .toSorted(([leftPattern], [rightPattern]) => {
      const kindOrder = Number(Object.hasOwn(defaults, leftPattern)) - Number(Object.hasOwn(defaults, rightPattern));
      return kindOrder || leftPattern.localeCompare(rightPattern, undefined, { sensitivity: "base" });
    })
    .map(([pattern, replacement]) => cocopiStatusInstructionReplacementRuleHtml(pattern, replacement, defaults[pattern]))
    .join("\n");
}

/**
 * @param {import("./instruction-replacements.js").CocopiInstructionReplacementSnapshot | undefined} snapshot
 * @param {string} defaultLabel
 */
function cocopiStatusInstructionReplacementTextsHtml(snapshot, defaultLabel) {
  const entries = snapshot?.entries?.length
    ? snapshot.entries.toSorted((left, right) => {
      const modificationOrder = Number(left.original === left.rewritten) - Number(right.original === right.rewritten);
      return modificationOrder || left.label.localeCompare(right.label, undefined, { sensitivity: "base" });
    })
    : [{ label: "", original: snapshot?.original ?? "", rewritten: snapshot?.rewritten ?? "" }];
  return entries.map((entry) => {
    const label = entry.label || defaultLabel;
    return `
          <article class="replacement-text" data-replacement-text data-replacement-label="${escapeHtml(label)}">
            ${entry.label ? `<div class="replacement-text-head"><h3>${escapeHtml(entry.label)}</h3><span class="replacement-text-spacer"></span><span class="replacement-text-status replacement-text-matched" data-replacement-text-matched hidden>Matched</span><span class="replacement-text-status replacement-text-modified" data-replacement-text-modified hidden>Modified</span></div>` : ""}
            <div class="replacement-columns">
              <label>
                Source text
                <textarea data-replacement-source spellcheck="false" placeholder="Paste text to test these rules.">${escapeHtml(entry.original)}</textarea>
              </label>
              <label>
                Live replacement preview
                <textarea data-replacement-preview readonly spellcheck="false">${escapeHtml(entry.rewritten)}</textarea>
              </label>
            </div>
          </article>`;
  }).join("\n");
 ...
 * @param {string | undefined} defaultReplacement
 */
function cocopiStatusInstructionReplacementRuleHtml(pattern, replacement, defaultReplacement) {
  const builtIn = defaultReplacement !== undefined;
  const enabled = !builtIn || replacement !== "";
  const displayedReplacement = builtIn && !enabled ? defaultReplacement : replacement;
  return `
            <article class="replacement-rule">
              <div class="replacement-rule-head">
                ${builtIn ? `<label class="switch"><input type="checkbox" data-replacement-enabled${enabled ? " checked" : ""}> Enabled</label><span class="field-hint">Built-in</span>` : '<span class="field-hint">Custom</span>'}
                <span class="replacement-rule-spacer"></span>
                <span class="replacement-match" data-replacement-match hidden>Matched</span>
                ${builtIn ? "" : '<button type="button" data-remove-replacement>Remove</button>'}
              </div>
              <label>Regex pattern<textarea class="replacement-rule-pattern" data-replacement-pattern${builtIn ? " readonly" : ""} spellcheck="false">${escapeHtml(pattern)}</textarea></label>
              <label>Replacement text<textarea data-replacement-value${builtIn ? "" : ' placeholder="(Remove matched text)"'} spellcheck="false">${escapeHtml(displayedReplacement)}</textarea></label>
            </article>`;
}
 ...
 * @param {StatusPanelFeatureAuditItem[]} featureAudit
 */
function cocopiStatusAutocompleteHtml(runtime, models, featureAudit) {
  const configuration = runtime.configuration;
  const inline = configuration.inlineCompletions;
  const route = resolveCocopiWorkloadRoute(COCOPI_WORKLOAD_ROUTE_NAMES.autocomplete, configuration, models);
  const selectedModel = configuration.routes.autocomplete.model || inline.model;
  const inlineAudit = featureAudit.find((item) => item.title === "Inline autocomplete");
  const hostInlineSetting = inlineAudit?.settings.find((setting) => setting.key === "editor.inlineSuggest.enabled");
  const hostInlineEnabled = hostInlineSetting?.value !== false;
  return `
    <section class="section-intro" aria-label="Autocomplete overview">
      <h2>Inline autocomplete</h2>
      <p>Generates editor ghost text independently of the model selected for Chat.</p>
    </section>
      ...
      <article class="control-card">
        ...
        <label>
          ...
          <select name="inlineCompletionChoice">
            ${statusPanelInlineCompletionOptionsHtml(models, inline.enabled, selectedModel)}
          </select>
        </label>
        <p class="field-hint"><strong>Auto</strong> prefers a Spark-like low-latency model available to the signed-in account, then Luna. A specific available model pins the autocomplete target.</p>
      </article>
      <article class="control-card">
        <h2>Effective state</h2>
        <div class="state-strip" aria-label="Autocomplete status">
          <div class="state-item"><span class="state-label">Cocopi</span><strong>${inline.enabled ? "On" : "Off"}</strong></div>
          <div class="state-item"><span class="state-label">VS Code inline suggestions</span><strong>${hostInlineEnabled ? "On" : "Off"}</strong></div>
          <div class="state-item"><span class="state-label">Codex target</span><strong>${escapeHtml(route.targetModel)}</strong></div>
        </div>
        <p class="field-hint">Accept ghost text with the configured inline-suggestion keybinding, commonly Tab. <a href="#" data-setting="editor.inlineSuggest.enabled">Open VS Code settings</a></p>
      </article>
    </div>
    <details class="nested-disclosure">
      <summary>Advanced autocomplete route and request limits</summary>
      <div class="nested-disclosure-body">
        <p class="field-hint">The Autocomplete Mode above controls the model. Advanced settings control reasoning, service tier, context size, and timeout.</p>
        ${cocopiStatusWorkloadRouteHtml(COCOPI_WORKLOAD_ROUTE_NAMES.autocomplete, runtime, models)}
        <div class="field-grid">
          <label>Text before cursor<input type="number" min="0" name="inlineCompletions.maxPrefixCharacters" value="${htmlTableCell(String(inline.maxPrefixCharacters))}"></label>
          <label>Text after cursor<input type="number" min="0" name="inlineCompletions.maxSuffixCharacters" value="${htmlTableCell(String(inline.maxSuffixCharacters))}"></label>
          <label>Idle timeout ms<input type="number" min="0" name="inlineCompletions.timeoutMs" value="${htmlTableCell(String(inline.timeoutMs ?? 0))}"></label>
        </div>
        <p class="field-hint">No prompt-cache key is sent for autocomplete requests.</p>
      </div>
    </details>`;
}
 ...
 * @param {import("./runtime.js").CocopiRuntime} runtime
 * @param {import("../../data/Codex.js").CodexModelSummary[]} models
 * @param {StatusPanelUtilityRouting | undefined} utilityRouting
 */
function cocopiStatusChatRoutingHtml(runtime, models, utilityRouting) {
  return `
    <div class="control-grid">
      <article class="control-card">
        <h2>Chat fallback model</h2>
        <label>
          Model
          <select name="model">
            ${statusPanelModelSelectOptionsHtml(models, runtime.configuration.model)}
          </select>
        </label>
        <p class="field-hint">Used only when no selected Cocopi chat model is available. Changes apply immediately.</p>
      </article>
      ${cocopiStatusConversationContextHtml()}
    </div>
    ${utilityRouting ? cocopiStatusUtilityRoutingHtml(utilityRouting, models) : ""}
    <details class="nested-disclosure">
      <summary>Advanced utility route tuning</summary>
      <div class="nested-disclosure-body">
        <p class="field-hint">These targets, reasoning levels, and service tiers back the specialized utility models selected above.</p>
        ${cocopiStatusWorkloadRouteHtml(COCOPI_WORKLOAD_ROUTE_NAMES.utility, runtime, models)}
        ${cocopiStatusWorkloadRouteHtml(COCOPI_WORKLOAD_ROUTE_NAMES.utilitySmall, runtime, models)}
      </div>
    </details>`;
}

/** @param {import("./runtime.js").CocopiRuntime} runtime */
function cocopiStatusDiagnosticsSettingsHtml(runtime) {
  return `
    <article class="control-card">
      <h2>Diagnostics</h2>
      <label>
        ...
        <select name="debugLevel">
          ${statusPanelSelectOptionHtml("off", "Off", runtime.configuration.debugLevel)}
          ${statusPanelSelectOptionHtml("metadata", "Metadata", runtime.configuration.debugLevel)}
          ${statusPanelSelectOptionHtml("events", "Events", runtime.configuration.debugLevel)}
          ${statusPanelSelectOptionHtml("payloads", "Payloads", runtime.configuration.debugLevel)}
        </select>
      </label>
...
/**
 * @param {import("./workload-routes.js").CocopiWorkloadRouteName} routeName
 * @param {import("./runtime.js").CocopiRuntime} runtime
 * @param {import("../../data/Codex.js").CodexModelSummary[]} models
 */
function cocopiStatusWorkloadRouteHtml(routeName, runtime, models) {
  const route = resolveCocopiWorkloadRoute(routeName, runtime.configuration, models);
  const configurationKey = routeName === COCOPI_WORKLOAD_ROUTE_NAMES.utilitySmall ? "utilitySmall" : routeName;
  const configured = runtime.configuration.routes[configurationKey];
  const labels = {
    utility: "Utility",
    "utility-small": "Utility small",
    autocomplete: "Autocomplete"
  };
  const modelOptions = statusPanelModelOptions(models, route.targetModel);
  const selectedTarget = routeName === COCOPI_WORKLOAD_ROUTE_NAMES.autocomplete && !configured.model
    ? ""
    : (configured.model || route.targetModel);
  const targetControl = routeName === COCOPI_WORKLOAD_ROUTE_NAMES.autocomplete
    ? ""
    : `<label>Target
              <select name="routes.${escapeHtml(configurationKey)}.model">
                ${statusPanelSelectOptionHtml("auto", "Auto", configured.model)}
                ${modelOptions.map((model) => statusPanelSelectOptionHtml(model.id, statusPanelModelOptionLabel(model), selectedTarget)).join("")}
              </select>
            </label>`;
  return `
        <div class="workload-route" data-workload-route="${escapeHtml(routeName)}">
          <h3>${escapeHtml(labels[routeName])} <code>${escapeHtml(route.alias)}</code></h3>
          <p class="field-hint">Resolved target: <strong>${escapeHtml(route.targetModel)}</strong></p>
          <div class="field-grid">
            ${targetControl}
            <label>Reasoning
              <select name="routes.${escapeHtml(configurationKey)}.reasoningEffort">
                ${["lowest", "default", "none", "minimal", "low", "medium", "high", "xhigh", "max"].map((effort) => statusPanelSelectOptionHtml(effort, effort === "lowest" ? "Lowest supported" : effort, configured.reasoningEffort)).join("")}
              </select>
            </label>
            <label>Service tier
              <select name="routes.${escapeHtml(configurationKey)}.serviceTier">
                ${["auto", "flex", "priority"].map((tier) => statusPanelSelectOptionHtml(tier, tier === "auto" ? "Default" : tier, configured.serviceTier)).join("")}
              </select>
            </label>
          </div>
        </div>`;
...
function cocopiStatusUtilityRoutingHtml(routing, models) {
  ...
  const stateClass = routing.state === "required" ? "required" : (routing.mode === "none" ? "off" : routing.state);
  const stateLabel = routing.state === "recommended"
    ? "Recommended active"
    : (routing.state === "required"
      ? "Setup required"
      : ({
          none: "Off",
          mainAgent: "Uses chat model",
          copilot: "Uses GitHub Copilot",
          specific: "Custom models",
  ...
  return `
    <section class="utility-panel control-card" aria-label="Background task models">
      <div class="utility-head">
        ...
        <span class="utility-state ${stateClass}">${stateLabel}</span>
      </div>
      <div class="utility-form">
        <label>
          Setup
          <select name="utilitySetupChoice">
            ${statusPanelSelectOptionHtml("recommended", "Recommended Cocopi workload routes", setupChoice)}
...
function statusPanelUtilitySettingsForChoice(choice, utilityModel, utilitySmallModel) {
  return {
    "chat.utilityModel": choice === "recommended" ? "cocopi/utility" : (choice === "specific" || choice === "external" ? utilityModel : ""),
    "chat.utilitySmallModel": choice === "recommended" ? "cocopi/utility-small" : (choice === "specific" || choice === "external" ? utilitySmallModel : ""),
    "chat.byokUtilityModelDefault": choice === "mainAgent" || choice === "copilot" ? choice : "none"
  };
}
...
function utilitySetupDescription(choice) {
  switch (choice) {
    case "recommended": {
      return "Use Cocopi's specialized general and small utility workload models.";
    }
    case "mainAgent": {
...
function statusPanelInlineCompletionOptionsHtml(models, enabled, currentModel) {
  ...
  return [
    ...
    statusPanelSelectOptionHtml(COCOPI_INLINE_COMPLETION_MODEL_AUTO, "Auto (Spark, then Luna)", current),
...
async function showTokenTrackerWindow(context, vscode) {
  ...
  let pending = [];
  /** @type {import("./token-cache-debug.js").CocopiWorkload | undefined} */
  let workloadFilter;
  /** @type {import("./token-cache-debug.js").CocopiWorkloadSubtype | undefined} */
  let workloadSubtypeFilter;
  ...
  const flushPendingSummaries = () => {
    ...
    void webview.postMessage({
      ...
      analyticsHtml: usageAnalyticsHtml(readCocopiUsageAnalytics({ timelineDays, workload: workloadFilter, workloadSubtype: workloadSubtypeFilter }), {
  ...
  if (typeof panel.webview.onDidReceiveMessage === "function") {
    panel.webview.onDidReceiveMessage((message) => {
      ...
      if (record.type === "updateTokenTrackerFilter") {
        workloadFilter = record.workload === "chat" || record.workload === "utility" || record.workload === "autocomplete" || record.workload === "unknown"
          ? record.workload
          : undefined;
        workloadSubtypeFilter = record.workloadSubtype === "main" || record.workloadSubtype === "general" || record.workloadSubtype === "small" || record.workloadSubtype === "inline" || record.workloadSubtype === "unknown"
          ? record.workloadSubtype
          : undefined;
        void panel.webview.postMessage?.({
  ...
  await refreshTokenCacheUsageStatus(context, vscode, panel.webview, {
    ...
    timeline: configuration.get("showTokenTrackerTimeline", true),
    timelineMode: configuration.get("tokenTrackerTimelineMode", COCOPI_TOKEN_TRACKER_TIMELINE_MODES.both),
    readAnalytics: () => readCocopiUsageAnalytics({ timelineDays, workload: workloadFilter, workloadSubtype: workloadSubtypeFilter })
  });
}
 ...
 * @param {{ timelineDays?: number, timeline?: boolean, timelineMode?: string, readAnalytics?: () => import("./token-cache-debug.js").CocopiUsageAnalytics }} [options]
 */
async function refreshTokenCacheUsageStatus(context, vscode, webview, options = {}) {
  ...
  await webview.postMessage({
    ...
    analyticsHtml: usageAnalyticsHtml(options.readAnalytics?.() ?? readCocopiUsageAnalytics({ timelineDays: options.timelineDays }), options)
  });
}
...
<head>
  ...
  <style>
    ...
    .workload-filter { align-items: end; display: flex; flex-wrap: wrap; gap: 6px; margin: 10px 0; }
    .workload-filter button[aria-pressed="true"] { background: var(--vscode-button-background); color: var(--vscode-button-foreground); }
    .workload-filter label { display: grid; gap: 3px; }
...
<body>
  ...
  <div class="workload-filter" aria-label="Token Tracker workload filter">
    <button type="button" data-workload-filter="all" aria-pressed="true">All</button>
    <button type="button" data-workload-filter="chat" aria-pressed="false">Chat</button>
    <button type="button" data-workload-filter="utility" aria-pressed="false">Utility</button>
    <button type="button" data-workload-filter="autocomplete" aria-pressed="false">Autocomplete</button>
    <label>Subtype
      <select id="tokenTrackerWorkloadSubtype">
        <option value="all">All subtypes</option>
        <option value="main">Main</option>
        <option value="general">General utility</option>
        <option value="small">Small utility</option>
  ...
  <script>
    ...
    let workloadSubtypeFilter = "all";
    const applyWorkloadFilter = () => {
      for (const group of document.querySelectorAll("details.conversation-group")) {
        let visibleEntries = 0;
        for (const entry of group.querySelectorAll("details.entry")) {
          if (!(entry instanceof HTMLElement)) {
            continue;
          }
          const visible = (workloadFilter === "all" || entry.dataset.workload === workloadFilter)
            && (workloadSubtypeFilter === "all" || entry.dataset.workloadSubtype === workloadSubtypeFilter);
          entry.hidden = !visible;
          visibleEntries += visible ? 1 : 0;
        }
    ...
    for (const button of document.querySelectorAll("[data-workload-filter]")) {
      button.addEventListener("click", () => {
        workloadFilter = button instanceof HTMLElement ? button.dataset.workloadFilter ?? "all" : "all";
        for (const candidate of document.querySelectorAll("[data-workload-filter]")) {
          candidate.setAttribute("aria-pressed", String(candidate === button));
        }
        applyWorkloadFilter();
        vscode.postMessage({
          type: "updateTokenTrackerFilter",
          workload: workloadFilter === "all" ? undefined : workloadFilter,
          workloadSubtype: workloadSubtypeFilter === "all" ? undefined : workloadSubtypeFilter
        });
      });
    }
    document.getElementById("tokenTrackerWorkloadSubtype")?.addEventListener("change", (event) => {
      workloadSubtypeFilter = event.target instanceof HTMLSelectElement ? event.target.value : "all";
      applyWorkloadFilter();
      vscode.postMessage({
        type: "updateTokenTrackerFilter",
        workload: workloadFilter === "all" ? undefined : workloadFilter,
    ...
    window.addEventListener("message", (event) => {
      const message = event.data;
      if (message?.type === "updateTokenCacheUsageStatus" && (typeof message.html === "string" || typeof message.analyticsHtml === "string")) {
        const usageStatus = document.getElementById("tokenCacheUsageStatus");
        if (usageStatus && typeof message.html === "string") {
...
function usageAnalyticsHtml(analytics, options = {}) {
  ...
  return `
    <section class="analytics-grid">
      ...
      ${usageAnalyticsWorkloadsHtml(analytics.workloads)}
      ...
      <div class="analytics-panel">
        <h3>This extension chat and session usage</h3>
...
/** @param {import("./token-cache-debug.js").CocopiWorkloadUsageSummary[]} workloads */
function usageAnalyticsWorkloadsHtml(workloads) {
  return `
      <div class="analytics-panel">
        <h3>Usage by workload</h3>
        ${workloads.length === 0 ? '<p class="analytics-note">No classified workload rows in this view.</p>' : `
        <table class="analytics-table">
          <thead><tr>${analyticsTableHeaderHtml("Workload", "Authoritative Cocopi workload classification.")}${analyticsTableHeaderHtml("Requests", "Requests in this workload.")}${analyticsTableHeaderHtml("Billable", "Uncached input plus output tokens.")}${analyticsTableHeaderHtml("Uncached input", "Input not reported as cached.")}${analyticsTableHeaderHtml("Output", "Generated output tokens.")}</tr></thead>
          <tbody>${workloads.map((workload) => `
            <tr><td><strong>${escapeHtml(workload.label)}</strong></td><td>${escapeHtml(formatTokenCacheNumber(workload.requestCount))}</td><td>${escapeHtml(formatTokenCacheTokenCount(workload.billableTokens))}</td><td>${escapeHtml(formatTokenCacheTokenCount(workload.uncachedInputTokens))}</td><td>${escapeHtml(formatTokenCacheTokenCount(workload.outputTokens))}</td></tr>
            ${workload.subtypes.map((subtype) => `<tr><td>↳ ${escapeHtml(subtype.label)}</td><td>${escapeHtml(formatTokenCacheNumber(subtype.requestCount))}</td><td>${escapeHtml(formatTokenCacheTokenCount(subtype.billableTokens))}</td><td>${escapeHtml(formatTokenCacheTokenCount(subtype.uncachedInputTokens))}</td><td>${escapeHtml(formatTokenCacheTokenCount(subtype.outputTokens))}</td></tr>`).join("")}`).join("")}</tbody>
        </table>`}
        <p class="analytics-note">Utility combines general and small utility routes; subtype rows provide drill-down.</p>
      </div>`;
}
...
function usageAnalyticsSessionsTableHtml(sessions) {
  if (sessions.length === 0) {
    return `<p class="empty">No chat/session rows retained yet.</p>`;
  }
  ...
  return `
    <table class="analytics-table">
      <thead>
        <tr>
          ${analyticsTableHeaderHtml("Chat/session", "Local Cocopi source and compact session identifier.")}
...
function tokenCacheSummaryDetailsHtml(entry) {
  ...
  return `
    <details class="entry" data-entry-id="${escapeHtml(String(entry.id))}" data-workload="${escapeHtml(entry.workload)}" data-workload-subtype="${escapeHtml(entry.workloadSubtype)}">
      ...
      <div class="entry-body">
        <div class="meta-grid">
          ...
          <div class="meta-row"><span class="meta-label">Workload</span><span>${escapeHtml(entry.workload)}</span></div>
          <div class="meta-row"><span class="meta-label">Workload subtype</span><span>${escapeHtml(entry.workloadSubtype)}</span></div>
          <div class="meta-row"><span class="meta-label">Requested model</span><span>${escapeHtml(entry.requestedModel)}</span></div>
          <div class="meta-row"><span class="meta-label">Resolved model</span><span>${escapeHtml(entry.resolvedModel)}</span></div>
...
/** @param {import("./token-cache-debug.js").CocopiTokenCacheDebugSource} source */
function formatTokenCacheSource(source) {
  if (source === "language-model") {
    return "Language Model";
  }
  return source === "inline-completion" ? "Inline Completion" : "Chat";
}
...
/**
 * @param {{ sessionId: string, conversationSummary?: string, conversationDescription?: string, source?: import("./token-cache-debug.js").CocopiTokenCacheDebugSource }} options
 * @returns {string}
 */
...
function cocopiInlineCompletionOptionsQuickPickItems(runtime) {
  ...
  return [
    ...
    },
    {
      ...
      detail: "Choose the model used for Cocopi AI autocomplete. Auto prefers an available Spark-like low-latency model, then Luna.",
