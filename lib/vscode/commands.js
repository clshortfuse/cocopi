...
export function registerCocopiCommands(context, vscode) {
  ...
  statusSurfaces = registerCocopiStatusSurfaces(context, vscode);
}
...
function registerCocopiStatusSurfaces(context, vscode) {
  /** @type {CocopiStatusSurfaceController[]} */
  const controllers = [];
  for (const registerSurface of [registerCocopiStatusBar, registerCocopiChatStatusItem]) {
    try {
      const controller = registerSurface(context, vscode);
      if (controller) {
        controllers.push(controller);
      }
    } catch {
      // Keep Cocopi commands available even if a status surface API is unavailable at runtime.
    }
  }
