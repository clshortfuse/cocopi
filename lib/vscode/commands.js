...
async function showCocopiStatusWindow(context, vscode) {
  ...
  const render = async () => {
    ...
    panel.webview.html = cocopiStatusWindowHtml(snapshot.auth, snapshot.runtime, snapshot.usage, snapshot.issueCount, snapshot.featureAudit, snapshot.models);
  };
  await render();
 ...
 */
function cocopiStatusWindowHtml(auth, runtime, usage, issueCount, featureAudit, models = []) {
