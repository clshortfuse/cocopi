...
import { codexContinuationAnchorFromInputItems, createCodexContinuationInputDigestBuilder, isCodexPreviousResponseNotFoundError } from "../codex-api/websocket.js";
...
/** @typedef {{ responseItems: CodexResponseInputItem[], sessionId?: string, hostRequestIndex?: number, responseId?: string, baselineItems?: number, baselineDigest?: string, requestState?: Record<string, CodexJsonValue> }} StatefulMarkerState */
...
function estimatedStatefulMarkerCharacters(part) {
  ...
  // prompt text; decoding legacy v1 markers here can temporarily materialize
  // huge duplicated response item arrays before Cocopi is even asked to send a
  // request. Request construction still decodes legacy markers when needed.
  return 0;
}
...
function pushLanguageModelMessage(input, role, message, modelId, messageIndex, options) {
  ...
  if (state) {
    const continuationAnchors = options.continuationAnchors;
    if (!state.baselineDigest && state.responseId && state.requestState && continuationAnchors) {
      pushRestoredContinuationAnchor(continuationAnchors, {
        ...codexContinuationAnchorFromInputItems(state.responseId, input, state.responseItems),
...
function pruneDuplicateStatefulMarkerResponseItems(message, role, responseItems) {
  ...
  const visibleAssistantText = role === "assistant" ? textFromLanguageModelMessage(message).trim() : "";
...
function encodeCocopiStatefulMarkerMetadataPayload(sessionId, options = {}) {
  return `${COCOPI_STATEFUL_MARKER_METADATA_PREFIX}${base64UrlEncodeUtf8(canonicalCodexJsonString({
    version: 2,
    sessionId,
...
function decodeCocopiStatefulMarkerPayload(marker, options) {
  try {
    const metadataJson = decodeStatefulMarkerPayloadJson(marker, COCOPI_STATEFUL_MARKER_METADATA_PREFIX);
    if (metadataJson) {
      const state = statefulMarkerPayloadRecordFromJson(metadataJson, options);
      return state ? decodeCocopiStatefulMarkerMetadataRecord(state, options) : undefined;
    }

    const json = decodeStatefulMarkerPayloadJson(marker, COCOPI_STATEFUL_MARKER_PAYLOAD_PREFIX);
    if (!json) {
      return;
    }
    const state = statefulMarkerPayloadRecordFromJson(json, options);
    return state ? decodeLegacyCocopiStatefulMarkerPayloadRecord(state, options) : undefined;
  } catch (error) {
 ...
 * @param {string} prefix
 */
function decodeStatefulMarkerPayloadJson(marker, prefix) {
  return marker.startsWith(prefix)
    ? base64UrlDecodeUtf8(marker.slice(prefix.length))
    : undefined;
}
 ...
 * @returns {StatefulMarkerState | undefined}
 */
function decodeLegacyCocopiStatefulMarkerPayloadRecord(state, options) {
  ...
  const rawResponseItems = /** @type {CodexJsonValue[]} */ (state.responseItems);
  const responseItems = codexMarkerResponseInputItems(rawResponseItems, options);
  return responseItems.length > 0 || metadata.sessionId ? {
    responseItems,
    ...metadata
  } : undefined;
}
...
function decodeCocopiStatefulMarkerMetadata(state, options) {
  ...
  return sessionId || hostRequestIndex || responseId || requestState || hasContinuationAnchor ? {
    ...(sessionId ? { sessionId } : {}),
    ...(hostRequestIndex ? { hostRequestIndex } : {}),
...
/**
 * @param {CodexJsonValue[]} rawResponseItems
 * @param {{ debugLevel?: 'off' | 'metadata' | 'events' | 'payloads', logger?: import("./diagnostics.js").CocopiLogger }} options
 */
function codexMarkerResponseInputItems(rawResponseItems, options) {
  /** @type {CodexResponseInputItem[]} */
  const responseItems = [];
