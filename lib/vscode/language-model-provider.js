...
import { createCodexContinuationInputDigestBuilder, isCodexPreviousResponseNotFoundError } from "../codex-api/websocket.js";
...
import { COCOPI_SERVICE_TIERS, codexReasoningFromCocopiOptions, codexServiceTierFromCocopiOptions, codexToolOptionsFromCocopiOptions, cocopiUltraMultiAgentModeFromOptions, resolveChatParticipantInstructions, resolveCocopiUltraMultiAgentInstructions, resolveVscodeLanguageModelTools } from "./configuration.js";
import { logCodexFailurePayloadDiagnostics, logCodexRequestDiagnostics, logCodexResponseEventDiagnostics, logCodexTokenCacheSummary, logCodexWebSocketContinuationDecision, logCocopiMemoryDiagnostics, noopCocopiLogger, readCodexUsageSummary, summarizeCodexRequestBodyForDiagnostics } from "./diagnostics.js";
import { COCOPI_LANGUAGE_MODEL_VENDOR, registerFailClosedCocopiLanguageModelProvider } from "./fail-closed-language-model-provider.js";
...
import { createReasoningSummaryHtmlCommentSanitizer, isCocopiHiddenProgressPart } from "./visible-text.js";
import { COCOPI_WORKLOAD_ROUTE_NAMES, cocopiWorkloadRouteNameFromModelId, ordinaryCocopiWorkloadRoute, resolveCocopiWorkloadRoute } from "./workload-routes.js";

export { COCOPI_LANGUAGE_MODEL_VENDOR } from "./fail-closed-language-model-provider.js";
...
const COCOPI_SIGN_IN_COMMAND = "cocopi.signIn";
const COCOPI_STATEFUL_MARKER_CURRENT_VERSION = 3;
...
/** @type {readonly { prefix: string, version?: number, kind: "metadata" | "legacy-response-items" }[]} */
const COCOPI_STATEFUL_MARKER_PAYLOAD_PREFIXES = [
  { prefix: COCOPI_LEGACY_STATEFUL_MARKER_PAYLOAD_PREFIX, version: 1, kind: "legacy-response-items" },
  { prefix: COCOPI_LEGACY_STATEFUL_MARKER_METADATA_PREFIX, version: 2, kind: "metadata" },
  { prefix: COCOPI_VERSIONED_STATEFUL_MARKER_METADATA_PREFIX, version: 3, kind: "metadata" },
  { prefix: COCOPI_STATEFUL_MARKER_PREFIX, kind: "metadata" }
];
const COCOPI_STATEFUL_MARKER_METADATA_KEYS = ["sessionId", "responseId", "hostRequestIndex", "baselineItems", "baselineDigest"];
const COCOPI_STATEFUL_MARKER_DATA_SEPARATOR = "\\";
...
const LANGUAGE_MODEL_RESTORED_CONTINUATION_ANCHOR_LIMIT = 1;
...
const DEFAULT_MODEL_CONFIGURATION_REASONING_EFFORTS = /** @type {const} */ (["low", "medium", "high", "xhigh", "max", "ultra"]);
...
/** @typedef {import("./workload-routes.js").CocopiResolvedWorkloadRoute} CocopiResolvedWorkloadRoute */
/** @typedef {import("../codex-api/websocket.js").CodexCompactContinuationAnchor} RestoredContinuationAnchor */
...
/** @typedef {{ responseItems: CodexResponseInputItem[], sessionId?: string, hostRequestIndex?: number, responseId?: string, baselineItems?: number, baselineDigest?: string }} StatefulMarkerState */
/** @typedef {{ json: string, version?: number, kind: "metadata" | "legacy-response-items" }} StatefulMarkerPayloadEnvelope */
 ...
 * @property {import("./configuration.js").ConfigurationApiLike["workspace"] & { onDidChangeConfiguration?: (listener: (event: { affectsConfiguration(section: string): boolean }) => void) => { dispose(): void } }} workspace
...
export function registerCocopiLanguageModelProvider(context, vscode, options = {}) {
  ...
  const boundary = registerFailClosedCocopiLanguageModelProvider(context, vscode, { logger });
  try {
    boundary.setDelegate(createCocopiLanguageModelProvider(context, vscode, options));
  } catch (error) {
    logger.error("Cocopi provider implementation failed to initialize; fail-closed provider remains active.", normalizeCaughtError(error));
  }
  void prewarmCocopiLanguageModelProvider(vscode, logger);
}
...
export function createCocopiLanguageModelProvider(context, vscode, options = {}) {
  ...
  const routeConfigurationSubscription = subscribeToWorkloadRouteConfigurationChanges(vscode, () => {
    loggedLanguageModelCompactionDiagnostics.clear();
    modelInformationChanged.fire();
  });
  if (routeConfigurationSubscription && "subscriptions" in context && Array.isArray(context.subscriptions)) {
    context.subscriptions.push(routeConfigurationSubscription);
  }
  ...
  return {
    ...
    async provideLanguageModelChatInformation(options, _token) {
      ...
      if (!runtime.auth) {
        ...
        return logLanguageModelCompactionDiagnostics(logger, runtime.configuration.debugLevel, languageModelInformationWithWorkloadRoutes([genericLanguageModelInformation(runtime.configuration.model, "Sign in required", compactionOptionsFromConfiguration(runtime.configuration))], [], runtime.configuration, compactionOptionsFromConfiguration(runtime.configuration)), [], compactionOptionsFromConfiguration(runtime.configuration), loggedLanguageModelCompactionDiagnostics);
      }
      ...
      if (modelCatalogCache?.key === modelCatalogCacheKey(runtime)) {
        if (modelCatalogCache.expiresAtMs > Date.now()) {
          return logLanguageModelCompactionDiagnostics(logger, runtime.configuration.debugLevel, languageModelInformationWithWorkloadRoutes(languageModelInformationFromCodexModels(modelCatalogCache.models, runtime.configuration.model, compactionOptionsFromConfiguration(runtime.configuration)), modelCatalogCache.models, runtime.configuration, compactionOptionsFromConfiguration(runtime.configuration)), modelCatalogCache.models, compactionOptionsFromConfiguration(runtime.configuration), loggedLanguageModelCompactionDiagnostics);
        }
      ...
      if (options.silent) {
        scheduleModelCatalogRefresh(context, runtime, logger, vscode);
        return logLanguageModelCompactionDiagnostics(logger, runtime.configuration.debugLevel, languageModelInformationWithWorkloadRoutes([genericLanguageModelInformation(runtime.configuration.model, "Model catalog loading", compactionOptionsFromConfiguration(runtime.configuration))], [], runtime.configuration, compactionOptionsFromConfiguration(runtime.configuration)), [], compactionOptionsFromConfiguration(runtime.configuration), loggedLanguageModelCompactionDiagnostics);
      }
      ...
      try {
        ...
        return logLanguageModelCompactionDiagnostics(logger, runtime.configuration.debugLevel, languageModelInformationWithWorkloadRoutes(languageModelInformationFromCodexModels(models.catalog, runtime.configuration.model, compactionOptionsFromConfiguration(runtime.configuration)), models.catalog, runtime.configuration, compactionOptionsFromConfiguration(runtime.configuration)), models.catalog, compactionOptionsFromConfiguration(runtime.configuration), loggedLanguageModelCompactionDiagnostics);
      } catch (error) {
        ...
        return logLanguageModelCompactionDiagnostics(logger, runtime.configuration.debugLevel, languageModelInformationWithWorkloadRoutes([genericLanguageModelInformation(runtime.configuration.model, "Model catalog unavailable", compactionOptionsFromConfiguration(runtime.configuration))], [], runtime.configuration, compactionOptionsFromConfiguration(runtime.configuration)), [], compactionOptionsFromConfiguration(runtime.configuration), loggedLanguageModelCompactionDiagnostics);
      }
    },
    ...
    async provideLanguageModelChatResponse(model, messages, requestOptions, progress, token) {
      ...
      const workloadRouteName = cocopiWorkloadRouteNameFromModelId(receivedModelId);
      const workloadRoute = workloadRouteName
        ? resolveCocopiWorkloadRoute(workloadRouteName, runtime.configuration, modelCatalogCache?.models)
        : ordinaryCocopiWorkloadRoute(receivedModelId, cocopiLanguageModelRequestModel(receivedModelId).modelId);
      const requestedModel = workloadRouteName
        ? { modelId: workloadRoute.targetModel }
        : cocopiLanguageModelRequestModel(receivedModelId);
      ...
      try {
        ...
        const taskCompletion = terminalTaskCompletion(requestState.input);
        if (taskCompletion) {
          let completionText = "";
          if (taskCompletion.summary && !taskCompletion.alreadyVisible) {
            completionText = taskCompletion.summary;
          } else if (taskCompletion.alreadyVisible) {
            completionText = "Task completed.";
          }
          if (completionText) {
            progress.report(new vscode.LanguageModelTextPart(completionText));
          }
          if (taskCompletion.summary || taskCompletion.alreadyVisible) {
            logger.info(`Language model completed after terminal task_complete result. source=language-model summaryPresent=${Boolean(taskCompletion.summary)} alreadyVisible=${taskCompletion.alreadyVisible}`);
            return;
          }
          logger.info("Language model terminal task_complete result had no visible summary; requesting a model follow-up. source=language-model");
        }
        const baseRequestInstructions = resolveLanguageModelInstructions(requestState.instructions, runtime.configuration);
        ...
        logCocopiMemoryDiagnostics(logger, runtime.configuration.debugLevel, {
          ...
          inputItems: requestState.input.length,
          ...
          cocopiDataParts: messageSummary.cocopiDataParts,
          cocopiDataBytes: messageSummary.cocopiDataBytes,
          continuationAnchors: requestState.continuationAnchors?.length ?? 0,
        ...
        const requestTools = resolveVscodeLanguageModelTools(userTools, runtime.configuration);
        const tools = codexToolsFromLanguageModelTools(requestTools, toolOptions);
        ...
        const requestModelCatalogInfo = modelCatalogCache?.models.find((candidate) => candidate.id === requestModelId);
        const effectiveModelOptions = workloadRouteName
          ? { ...modelOptions, reasoningEffort: workloadRoute.reasoningEffort ?? "default" }
          : modelOptions;
        const reasoning = codexReasoningFromCocopiOptions(runtime.configuration, effectiveModelOptions, modelReasoningOptions);
        const ultraMultiAgentMode = cocopiUltraMultiAgentModeFromOptions(runtime.configuration, effectiveModelOptions, requestTools, {
          multiAgentVersion: requestModelCatalogInfo?.multiAgentVersion
        });
        const parallelToolCalls = (tools.length > 1 || ultraMultiAgentMode)
          && requestModelCatalogInfo?.supportsParallelToolCalls !== false;
        const requestInstructions = resolveCocopiUltraMultiAgentInstructions(baseRequestInstructions, ultraMultiAgentMode, { parallelToolCalls });
        const fastRequested = workloadRouteName
          ? workloadRoute.serviceTier === COCOPI_SERVICE_TIERS.priority
          : requestedModel.serviceTier === COCOPI_SERVICE_TIERS.priority || languageModelFastOptionSelected(modelOptions);
        const serviceTier = workloadRouteName
          ? workloadRoute.serviceTier
          : requestedModel.serviceTier ?? (fastRequested ? COCOPI_SERVICE_TIERS.priority : codexServiceTierFromCocopiOptions(runtime.configuration, modelOptions));
        const serviceTierSource = workloadRouteName ? "route" : (requestedModel.serviceTier ? "model" : "option");
        ...
        const continuationAnchorSummary = continuationAnchorDiagnosticsSummary(continuationAnchors);
        logCocopiMemoryDiagnostics(logger, runtime.configuration.debugLevel, {
          source: "language-model",
          hostRequestIndex,
          sessionId,
          ...
          tools: tools.length,
          continuationAnchors: continuationAnchorSummary.anchors,
          continuationAnchorInputItems: continuationAnchorSummary.inputItems,
          continuationAnchorResponseItems: continuationAnchorSummary.responseItems
        });
        tokenCacheSummaryContext = {
          source: "language-model",
          workload: workloadRoute.workload,
          workloadSubtype: workloadRoute.workloadSubtype,
          requestedModel: workloadRouteName ? workloadRoute.alias : receivedModelId,
        ...
        const visibleReplayAnchor = createVisibleReplayAnchorBuilder(requestState.input);
        /** @type {string[]} */
        const textDeltas = [];
        ...
        let messageTextDeltas = 0;
        let lastTextDeltaOutputIndex;
        ...
        const reasoningProgress = createLanguageModelReasoningProgressReporter(progress, vscode, logger, true);
        /**
         * @param {string} callId
        ...
        try {
          ...
          if (delta) {
            ...
            const outputIndex = responseOutputItemOrderFromEvent(event);
            if (lastTextDeltaOutputIndex !== undefined && outputIndex !== undefined && outputIndex !== lastTextDeltaOutputIndex) {
              textDeltas.push("\n\n");
              visibleReplayAnchor.appendAssistantText("\n\n");
              textProgress.report("\n\n");
            }
            lastTextDeltaOutputIndex = outputIndex ?? lastTextDeltaOutputIndex;
            reasoningProgress.finish();
            textDeltas.push(delta);
            visibleReplayAnchor.appendAssistantText(delta);
          ...
          if (reasoningDelta) {
            ...
            reasoningProgress.report(reasoningDelta, {
              ...
              stripTrailingStandaloneHtmlCommentParagraph: true
            });
            progressSummary.reasoningSummaryDeltas += 1;
          ...
          if (toolCall) {
            ...
            const toolCallItem = codexFunctionCallInputItemFromToolCall(toolCall);
            responseState.capture(toolCallItem, responseOutputItemOrderFromEvent(event));
            if (!reportedToolCallIds.has(toolCall.callId)) {
              reportedToolCallIds.add(toolCall.callId);
              visibleReplayAnchor.captureToolCall(toolCallItem);
              textProgress.finish();
              statefulMarkerEmitted = emitLanguageModelStatefulMarker({
                responseState,
                continuationAnchor: visibleReplayAnchor.continuationAnchor(streamResponseId),
        ...
        if (messageTextDeltas === 0 && fallbackOutputText) {
          ...
          visibleReplayAnchor.appendAssistantText(fallbackOutputText);
        ...
        logCocopiMemoryDiagnostics(logger, runtime.configuration.debugLevel, {
          ...
          model: requestModelId,
          inputItems: body.input?.length ?? 0,
          toolCalls: progressSummary.toolCalls
        });
...
function sanitizeStoredCodexModel(value) {
  ...
  return {
    ...
    ...(Array.isArray(value.serviceTiers) ? { serviceTiers: sanitizeStoredServiceTiers(value.serviceTiers) } : {}),
    ...(typeof value.defaultServiceTier === "string" && value.defaultServiceTier.length > 0 ? { defaultServiceTier: value.defaultServiceTier } : {}),
    ...
    ...(isCodexMultiAgentVersion(value.multiAgentVersion) ? { multiAgentVersion: value.multiAgentVersion } : {}),
    ...(isCodexToolMode(value.toolMode) ? { toolMode: value.toolMode } : {}),
    ...(typeof value.supportsParallelToolCalls === "boolean" ? { supportsParallelToolCalls: value.supportsParallelToolCalls } : {}),
...
/**
 * @param {unknown[]} tiers
 * @returns {import("../../data/Codex.js").CodexModelServiceTier[]}
 */
function sanitizeStoredServiceTiers(tiers) {
  return tiers.flatMap((tier) => {
    if (!isPlainRecord(tier)) {
      return [];
    }

    const record = /** @type {Record<string, unknown>} */ (tier);
...
/** @param {*} value */
function isCodexMultiAgentVersion(value) {
  return value === "disabled" || value === "v1" || value === "v2";
}

/** @param {*} value */
function isCodexToolMode(value) {
  return value === "direct" || value === "code_mode" || value === "code_mode_only";
}
/* eslint-enable jsdoc/reject-any-type */
 ...
 * @param {{ sessionId: string, hostRequestIndex: number, receivedModelId: string, requestModelId: string, modelOptions: Readonly<Record<string, unknown>> | undefined, serviceTier?: string, serviceTierSource: "model" | "option" | "route", reasoningEffort?: string, reasoningSummary?: string, fastRequested?: boolean }} options
...
/**
 * @param {{ receivedModelId: string, requestModelId: string, modelOptions: Readonly<Record<string, unknown>> | undefined, serviceTier?: string, serviceTierSource: "model" | "option" | "route", reasoningEffort?: string, reasoningSummary?: string, fastRequested?: boolean }} options
 */
 ...
 */
function createLanguageModelReasoningProgressReporter(progress, vscode, logger, visible, fallbackLabel = "Thinking") {
  let fallbackOpen = false;
  let nativeOpen = false;
  /** @type {string | undefined} */
  let nativeId;
  /** @type {{ new(value: string | string[], id?: string, metadata?: Record<string, unknown>): unknown } | undefined} */
  let NativeThinkingPart;
  try {
    NativeThinkingPart = vscode.LanguageModelThinkingPart;
  } catch (error) {
    logger.info(`Cocopi native language-model thinking parts are unavailable; using text fallback. error=${normalizeCaughtError(error).message}`);
  }
   ...
   */
  const reportNativePart = (value, id, metadata) => {
    if (!NativeThinkingPart) {
      return false;
    }
    try {
      progress.report(/** @type {import("vscode").LanguageModelResponsePart} */ (new NativeThinkingPart(value, id, metadata)));
      return true;
    } catch (error) {
      NativeThinkingPart = undefined;
      logger.info(`Cocopi native language-model thinking parts are unavailable; using text fallback. error=${normalizeCaughtError(error).message}`);
      return false;
    }
  };
  ...
  /**
   * @param {string} text
   * @param {{ id?: string, metadata?: Record<string, unknown>, nativeOnly?: boolean, stripTrailingStandaloneHtmlCommentParagraph?: boolean }} options
   */
  const reportPart = (text, options) => {
    if (!text) {
      return;
    }
    ...
    if (NativeThinkingPart) {
      if (nativeOpen && nativeId !== options.id) {
        reportNativePart("", "", { vscode_reasoning_done: true });
        nativeOpen = false;
      }
      if (reportNativePart(text, options.id, options.metadata)) {
        nativeOpen = true;
        nativeId = options.id;
        return;
      }
    ...
    if (!fallbackOpen) {
      progress.report(new vscode.LanguageModelTextPart(`<details open><summary>${fallbackLabel}</summary>\n\n`));
      fallbackOpen = true;
    }
    progress.report(new vscode.LanguageModelTextPart(text));
  };

  const sanitizer = createReasoningSummaryHtmlCommentSanitizer(reportPart);

  /**
   * @param {string} text
   * @param {{ id?: string, metadata?: Record<string, unknown>, nativeOnly?: boolean, stripTrailingStandaloneHtmlCommentParagraph?: boolean }} options
   */
  const reportSanitizedPart = (text, options) => {
    const id = options.id ?? "";
    if (sanitizedPartId !== undefined && sanitizedPartId !== id) {
      sanitizer.finish();
    }
    sanitizedPartId = id;
    sanitizer.append(text, options);
  };
  ...
  return {
    ...
    report(text, options = {}) {
      ...
      if (options.stripTrailingStandaloneHtmlCommentParagraph) {
        reportSanitizedPart(text, options);
      } else {
        sanitizer.finish();
        sanitizedPartId = undefined;
        reportPart(text, options);
      }
    },
...
function languageModelInformationVariantsFromCodexModel(model, modelIds, options = {}) {
  ...
  if (
    ...
    && (model.serviceTiers?.some((tier) => tier.id === "priority" || tier.id === "fast") || model.additionalSpeedTiers?.includes("fast"))
 ...
 * @param {{ useModelDefaultCompactionLimit?: boolean, compactionFallbackStrategy?: "full" | "ninety-percent" }} options
 */
function languageModelInformationWithWorkloadRoutes(information, models, configuration, options) {
  return [
    ...information,
    ...Object.values(COCOPI_WORKLOAD_ROUTE_NAMES).map((routeName) => {
      const route = resolveCocopiWorkloadRoute(routeName, configuration, models);
      const target = models.find((model) => model.id === route.targetModel);
      if (!target) {
        return specializedWorkloadLanguageModelInformation(
          genericLanguageModelInformation(route.targetModel, `Specialized ${route.workloadSubtype} workload model`, options),
          route
        );
      }
      return specializedWorkloadLanguageModelInformation(languageModelInformationFromCodexModel(target, options), route);
    })
  ];
}

/**
 * @param {import("vscode").LanguageModelChatInformation} target
 * @param {CocopiResolvedWorkloadRoute} route
 * @returns {import("vscode").LanguageModelChatInformation}
 */
function specializedWorkloadLanguageModelInformation(target, route) {
  const id = route.alias.slice(`${COCOPI_LANGUAGE_MODEL_VENDOR}/`.length);
  /** @type {Record<string, string>} */
  const names = {
    utility: "Cocopi Utility",
    "utility-small": "Cocopi Utility Small",
    autocomplete: "Cocopi Autocomplete"
  };
  const configurationSchema = specializedWorkloadConfigurationSchema(target.configurationSchema);
  const { configurationSchema: _targetConfigurationSchema, ...metadata } = target;
  void _targetConfigurationSchema;
  return {
    ...metadata,
    id,
    name: names[id] ?? id,
    version: target.version,
    detail: `Cocopi - Specialized ${route.workloadSubtype} workload → ${route.targetModel}`,
...
/**
 * Keep target context-size choices while route settings remain authoritative
 * for reasoning effort.
 *
 * @param {import("vscode").LanguageModelConfigurationSchema | undefined} schema
 * @returns {import("vscode").LanguageModelConfigurationSchema | undefined}
 */
function specializedWorkloadConfigurationSchema(schema) {
  if (!schema?.properties) {
    return;
  }
  const properties = Object.fromEntries(
    Object.entries(schema.properties).filter(([key]) => key !== LANGUAGE_MODEL_CONFIGURATION_OPTIONS_KEY)
  );
...
function uniqueReasoningChoices(choices) {
  ...
  return choices.filter((choice) => {
    if (!choice.effort || seen.has(choice.effort)) {
      return false;
    }
 ...
 * @returns {import("../codex-api/websocket.js").CodexCompactContinuationAnchor[]}
 */
function continuationAnchorsFromLanguageModelRequestState(requestState, body) {
  void body;
  return requestState.continuationAnchors ?? [];
}

/**
 * @param {import("../codex-api/websocket.js").CodexCompactContinuationAnchor[]} anchors
 */
function continuationAnchorDiagnosticsSummary(anchors) {
  let inputItems = 0;
  for (const anchor of anchors) {
    inputItems += anchor.baselineItems;
  }

  return { anchors: anchors.length, inputItems, responseItems: 0 };
}
...
function estimatedStatefulMarkerCharacters(part) {
  ...
  void marker;
  // This path is called by VS Code for token counting and chat compaction
  // heuristics. Hidden Cocopi markers are provider-private state, not visible
  // prompt text; decoding markers here would materialize provider-private
  // payloads before Cocopi is even asked to send a request.
  return 0;
}
...
function pushLanguageModelMessage(input, role, message, modelId, messageIndex, options) {
  const state = statefulMarkerStateFromLanguageModelMessage(message, role, modelId, messageIndex, options);
  if (state) {
  ...
  for (const part of message.content) {
    ...
    const text = visibleTextFromLanguageModelPart(part);
  ...
  flushContent();
  if (state?.responseId && state.baselineItems !== undefined && state.baselineDigest && options.continuationAnchors) {
    pushRestoredContinuationAnchor(options.continuationAnchors, {
...
/**
 * @param {RestoredContinuationAnchor[]} continuationAnchors
 * @param {RestoredContinuationAnchor} anchor
 */
function pushRestoredContinuationAnchor(continuationAnchors, anchor) {
  if (LANGUAGE_MODEL_RESTORED_CONTINUATION_ANCHOR_LIMIT === 1) {
    continuationAnchors.length = 0;
    continuationAnchors.push(anchor);
    return;
  }

  continuationAnchors.push(anchor);
  if (continuationAnchors.length > LANGUAGE_MODEL_RESTORED_CONTINUATION_ANCHOR_LIMIT) {
    continuationAnchors.splice(0, continuationAnchors.length - LANGUAGE_MODEL_RESTORED_CONTINUATION_ANCHOR_LIMIT);
  }
}
 ...
 * @returns {StatefulMarkerState | undefined}
 */
function statefulMarkerStateFromLanguageModelMessage(message, role, modelId, messageIndex, options) {
  if (!modelId) {
    return;
  }

  /** @type {StatefulMarkerState | undefined} */
  let latestState;
  for (const part of message.content) {
    ...
    if (!state) {
      logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "ignored", `reason=${unsupportedStatefulMarkerReason(marker.marker)} markerModelId=${marker.modelId} requestModelId=${modelId} markerBytes=${marker.marker.length} messageIndex=${messageIndex}`);
      continue;
    }

    const prunedResponseItems = pruneDuplicateStatefulMarkerResponseItems(message, role, state.responseItems);
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decoded", `markerModelId=${marker.modelId} requestModelId=${modelId} items=${prunedResponseItems.length} prunedItems=${state.responseItems.length - prunedResponseItems.length} sessionId=${state.sessionId ? "present" : "missing"} hostRequestIndex=${state.hostRequestIndex ? "present" : "missing"} responseId=${state.responseId ? "present" : "missing"} compactAnchor=${state.baselineItems !== undefined && state.baselineDigest ? "present" : "missing"} markerBytes=${marker.marker.length} messageIndex=${messageIndex}`);
    latestState = { ...state, responseItems: prunedResponseItems };
  }
 ...
 */
function pruneDuplicateStatefulMarkerResponseItems(message, role, responseItems) {
  if (responseItems.length === 0) {
    return responseItems;
  }

  const visibleAssistantText = role === "assistant" ? visibleTextFromLanguageModelMessage(message).trim() : "";
  const visibleToolCallIds = languageModelMessageToolCallIds(message);
  const visibleToolResultIds = languageModelMessageToolResultIds(message);
  return responseItems.filter((item) => {
    if (visibleAssistantText && isAssistantMessageInputItem(item)) {
      return false;
    }
    if (isFunctionCallInputItem(item) && visibleToolCallIds.has(item.call_id)) {
      return false;
    }
    if (isFunctionCallOutputInputItem(item) && visibleToolResultIds.has(item.call_id)) {
      return false;
    }
...
/** @param {import("vscode").LanguageModelChatRequestMessage} message */
function languageModelMessageToolCallIds(message) {
  /** @type {Set<string>} */
  const ids = new Set();
  for (const part of message.content) {
    const toolCall = codexFunctionCallInputItemFromLanguageModelPart(part);
    if (toolCall?.call_id) {
      ids.add(toolCall.call_id);
    }
  }
...
/** @param {import("vscode").LanguageModelChatRequestMessage} message */
function languageModelMessageToolResultIds(message) {
  /** @type {Set<string>} */
  const ids = new Set();
  for (const part of message.content) {
    const output = codexFunctionCallOutputInputItemFromLanguageModelPart(part);
    if (output?.call_id) {
      ids.add(output.call_id);
    }
  }
...
/**
 * Once VS Code returns a terminal task_complete result, Cocopi can render the
 * tool's summary payload directly instead of spending another model request on
 * a restatement.
 *
 * @param {CodexResponseInputItem[]} input
 * @returns {{ summary: string, alreadyVisible: boolean } | undefined}
 */
function terminalTaskCompletion(input) {
  const output = input.at(-1);
  if (!output || !isFunctionCallOutputInputItem(output)) {
    return;
  }

  for (let callIndex = input.length - 2; callIndex >= 0; callIndex -= 1) {
    const item = input[callIndex];
    if (isFunctionCallInputItem(item) && item.call_id === output.call_id) {
      ...
      for (let index = callIndex - 1; index >= 0; index -= 1) {
        const previousItem = input[index];
        if (isFunctionCallOutputInputItem(previousItem) || (previousItem && typeof previousItem === "object" && !Array.isArray(previousItem) && "role" in previousItem && previousItem.role === "user")) {
          segmentStart = index + 1;
          break;
        }
      }
      const visibleText = input.slice(segmentStart, callIndex).flatMap((previousItem) => {
        if (!previousItem || typeof previousItem !== "object" || Array.isArray(previousItem)) {
          return [];
        }
        return "role" in previousItem && previousItem.role === "assistant" && "content" in previousItem && Array.isArray(previousItem.content)
          ? previousItem.content.flatMap((part) => part && typeof part === "object" && "type" in part && part.type === "output_text" && "text" in part && typeof part.text === "string" ? [part.text] : [])
          : [];
      }).join("\n\n");
      try {
        const argumentsValue = JSON.parse(item.arguments);
        const summary = argumentsValue && typeof argumentsValue === "object" && !Array.isArray(argumentsValue) && typeof argumentsValue.summary === "string"
          ? argumentsValue.summary.trim()
          : "";
        return {
          summary,
          alreadyVisible: summary ? visibleText.includes(summary) : Boolean(visibleText.trim())
        };
      } catch {
        return { summary: "", alreadyVisible: Boolean(visibleText.trim()) };
      }
    }
  }
}
 ...
 * @param {RestoredContinuationAnchor | undefined} options.continuationAnchor
...
function emitLanguageModelStatefulMarker(options) {
  const responseItemCount = options.responseState.count(options.fallbackAssistantText);
  if (responseItemCount === 0 && !options.responseId) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "not-emitted", `reason=no-state emissionPoint=${options.emissionPoint}`);
    return false;
  }

  const marker = encodeCocopiStatefulMarkerMetadataPayload(options.sessionId, {
    responseId: options.responseId,
    hostRequestIndex: options.hostRequestIndex,
    responseItemCount,
    baselineItems: options.continuationAnchor?.baselineItems,
    baselineDigest: options.continuationAnchor?.baselineDigest
  });
  logProviderStatefulMarkerDiagnostics(
    options.logger,
    options.debugLevel,
    "emitted",
    [
      `mimeType=${COCOPI_STATEFUL_MARKER_MIME}`,
      `modelId=${options.requestModelId}`,
      `items=${responseItemCount}`,
      `compactAnchor=${options.continuationAnchor ? "present" : "missing"}`,
      `markerBytes=${marker.length}`,
      `emissionPoint=${options.emissionPoint}`
    ].join(" ")
  );
 ...
 * @param {import("./diagnostics.js").CocopiLogger | undefined} logger
 * @param {'off' | 'metadata' | 'events' | 'payloads' | undefined} debugLevel
 * @param {'emitted' | 'not-emitted' | 'decoded' | 'decoded-suspicious' | 'decode-failed' | 'ignored' | 'migrated'} event
...
/**
 * @returns {{ capture(item: CodexResponseInputItem, order?: number): void, count(fallbackAssistantText?: string): number }}
 */
function createResponseStateBuilder() {
  ...
  return {
    capture(item, order) {
      ...
      void order;
      itemCount += 1;
      hasAssistantMessage ||= isAssistantMessageInputItem(item);
    },
    count(fallbackAssistantText = "") {
      if (hasAssistantMessage || !codexAssistantMessageInputItemFromText(fallbackAssistantText)) {
        return itemCount;
      }
...
/**
 * Tracks the replay prefix VS Code can reconstruct from visible chat parts.
 * Hidden markers persist only this compact count+digest anchor, not the replay
 * items themselves.
 *
 * @param {CodexResponseInputItem[]} requestInput
 * @returns {{ appendAssistantText(text: string): void, captureToolCall(item: CodexResponseInputItem): void, continuationAnchor(responseId: string | undefined): RestoredContinuationAnchor | undefined }}
 */
function createVisibleReplayAnchorBuilder(requestInput) {
  const digest = createCodexContinuationInputDigestBuilder([requestInput]);
  let pendingAssistantText = "";
  const flushAssistantText = () => {
    const message = codexAssistantMessageInputItemFromText(pendingAssistantText);
  ...
  return {
    ...
    captureToolCall(item) {
      flushAssistantText();
      digest.update(item);
    },
    continuationAnchor(responseId) {
      if (!responseId) {
        return;
      }
...
async function showModelCatalogWarning(vscode, message, onSignInComplete) {
  ...
  if (selection === "Sign In") {
    await vscode.commands?.executeCommand?.(COCOPI_SIGN_IN_COMMAND);
    onSignInComplete?.();
  }
}
 ...
 */
function subscribeToWorkloadRouteConfigurationChanges(vscode, listener) {
  const onDidChangeConfiguration = vscode.workspace.onDidChangeConfiguration;
  if (typeof onDidChangeConfiguration !== "function") {
    return;
  }

  return onDidChangeConfiguration((event) => {
    if (
      event.affectsConfiguration("cocopi.routes")
      || event.affectsConfiguration("cocopi.inlineCompletions.model")
      || event.affectsConfiguration("cocopi.model")
    ) {
      listener();
    }
  });
...
/**
 * @param {string} sessionId
 * @param {{ responseId?: string, hostRequestIndex?: number, responseItemCount?: number, baselineItems?: number, baselineDigest?: string }} [options]
 */
function encodeCocopiStatefulMarkerMetadataPayload(sessionId, options = {}) {
  return `${COCOPI_STATEFUL_MARKER_PREFIX}${base64UrlEncodeUtf8(canonicalCodexJsonString({
    version: COCOPI_STATEFUL_MARKER_CURRENT_VERSION,
    sessionId,
    ...
    ...(isPositiveInteger(options.responseItemCount) ? { responseItemCount: options.responseItemCount } : {}),
    ...(isNonNegativeSafeInteger(options.baselineItems) && isContinuationDigest(options.baselineDigest) ? {
      baselineItems: options.baselineItems,
...
function decodeCocopiStatefulMarkerPayload(marker, options) {
  try {
    const envelope = decodeStatefulMarkerPayloadEnvelope(marker);
    if (!envelope) {
      return;
    }
    const state = statefulMarkerPayloadRecordFromJson(envelope.json, options);
    const migratedState = state ? migrateCocopiStatefulMarkerPayloadRecord(state, envelope, options) : undefined;
    return migratedState ? decodeCocopiStatefulMarkerMetadataRecord(migratedState, options) : undefined;
  } catch (error) {
...
/**
 * @param {string} marker
 * @returns {StatefulMarkerPayloadEnvelope | undefined}
 */
function decodeStatefulMarkerPayloadEnvelope(marker) {
  for (const payloadPrefix of COCOPI_STATEFUL_MARKER_PAYLOAD_PREFIXES) {
    if (marker.startsWith(payloadPrefix.prefix)) {
      return {
        json: base64UrlDecodeUtf8(marker.slice(payloadPrefix.prefix.length)),
        version: payloadPrefix.version,
        kind: payloadPrefix.kind
      };
    }
  }
}
 ...
 * @param {string} json
 * @param {{ debugLevel?: 'off' | 'metadata' | 'events' | 'payloads', logger?: import("./diagnostics.js").CocopiLogger }} options
 * @returns {Record<string, CodexJsonValue | undefined> | undefined}
 */
function statefulMarkerPayloadRecordFromJson(json, options) {
  ...
  return /** @type {Record<string, CodexJsonValue | undefined>} */ (value);
}

/**
 * @param {Record<string, CodexJsonValue | undefined>} state
 * @param {StatefulMarkerPayloadEnvelope} envelope
 * @param {{ debugLevel?: 'off' | 'metadata' | 'events' | 'payloads', logger?: import("./diagnostics.js").CocopiLogger }} options
 * @returns {Record<string, CodexJsonValue | undefined> | undefined}
 */
function migrateCocopiStatefulMarkerPayloadRecord(state, envelope, options) {
  const payloadVersion = isPositiveInteger(state.version) ? state.version : envelope.version;
  if (!payloadVersion) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", "reason=missing-state-version");
    return;
  }

  if (envelope.version && payloadVersion !== envelope.version) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", `reason=state-version-mismatch prefixVersion=${envelope.version} payloadVersion=${payloadVersion}`);
    return;
  }

  if (payloadVersion > COCOPI_STATEFUL_MARKER_CURRENT_VERSION) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", `reason=unsupported-state-version version=${payloadVersion}`);
    return;
  }

  if (payloadVersion === 1 && envelope.kind !== "legacy-response-items") {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", "reason=invalid-state-shape");
    return;
  }

  const migratedState = metadataOnlyStatefulMarkerRecord(state);
  if (!migratedState) {
    return;
  }

  if (payloadVersion !== COCOPI_STATEFUL_MARKER_CURRENT_VERSION) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "migrated", `fromVersion=${payloadVersion} toVersion=${COCOPI_STATEFUL_MARKER_CURRENT_VERSION} droppedResponseItems=${payloadVersion === 1 ? "true" : "false"}`);
  }
 ...
 * @returns {Record<string, CodexJsonValue | undefined> | undefined}
 */
function metadataOnlyStatefulMarkerRecord(state) {
  /** @type {Record<string, CodexJsonValue | undefined>} */
  const metadata = { version: COCOPI_STATEFUL_MARKER_CURRENT_VERSION };
  let metadataFields = 0;
  for (const key of COCOPI_STATEFUL_MARKER_METADATA_KEYS) {
  ...
  return metadataFields > 0 ? metadata : undefined;
}
 ...
 * @returns {StatefulMarkerState | undefined}
 */
function decodeCocopiStatefulMarkerMetadataRecord(state, options) {
  if (state.version !== COCOPI_STATEFUL_MARKER_CURRENT_VERSION) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", "reason=invalid-state-shape");
    return;
  }

  const metadata = decodeCocopiStatefulMarkerMetadata(state, options);
  return metadata ? { responseItems: [], ...metadata } : undefined;
}

/**
 * @param {Record<string, CodexJsonValue | undefined>} state
 * @param {{ debugLevel?: 'off' | 'metadata' | 'events' | 'payloads', logger?: import("./diagnostics.js").CocopiLogger }} options
 * @returns {Omit<StatefulMarkerState, "responseItems"> | undefined}
 */
function decodeCocopiStatefulMarkerMetadata(state, options) {
  ...
  const baselineItems = isNonNegativeSafeInteger(state.baselineItems)
    ? state.baselineItems
    : undefined;
  if (state.baselineItems !== undefined && baselineItems === undefined) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", "reason=invalid-baseline-items");
  }
  const baselineDigest = isContinuationDigest(state.baselineDigest)
    ? state.baselineDigest
    : undefined;
  if (state.baselineDigest !== undefined && !baselineDigest) {
    logProviderStatefulMarkerDiagnostics(options.logger, options.debugLevel, "decode-failed", "reason=invalid-baseline-digest");
  }
  const hasContinuationAnchor = Boolean(responseId && baselineItems !== undefined && baselineDigest);

  return sessionId || hostRequestIndex || responseId || hasContinuationAnchor ? {
    ...(sessionId ? { sessionId } : {}),
    ...(hostRequestIndex ? { hostRequestIndex } : {}),
    ...(responseId ? { responseId } : {}),
    ...(hasContinuationAnchor ? {
      baselineItems: /** @type {number} */ (baselineItems),
      baselineDigest: /** @type {string} */ (baselineDigest)
    } : {})
  } : undefined;
}
...
/** @param {import("../../data/Codex.js").CodexJsonValue | undefined} value */
function isContinuationDigest(value) {
  return typeof value === "string" && /^sha256:[\da-f]{64}$/u.test(value);
}
...
function encodeStatefulMarker(modelId, marker) {
  return TEXT_ENCODER.encode(`${modelId}${COCOPI_STATEFUL_MARKER_DATA_SEPARATOR}${marker}`);
}
...
function decodeStatefulMarkerDataPart(data) {
  ...
  const separatorIndex = text.indexOf(COCOPI_STATEFUL_MARKER_DATA_SEPARATOR);
  if (separatorIndex === -1) {
    return;
  }
 ...
 * @param {import("vscode").LanguageModelChatRequestMessage} message
 */
function visibleTextFromLanguageModelMessage(message) {
  return message.content
    .map((part) => visibleTextFromLanguageModelPart(part))
    .filter(Boolean)
    .join("\n\n");
}
 ...
 * @param {import("vscode").LanguageModelChatRequestMessage["content"][number]} part
 */
function visibleTextFromLanguageModelPart(part) {
  return isCocopiHiddenProgressPart(part) ? "" : textFromLanguageModelPart(part);
}
