...
import { codexReasoningSummaryMetadata, codexReasoningSummaryPartId, codexReasoningTextMetadata, codexReasoningTextPartId, readCodexReasoningSummaryTextDelta, readCodexReasoningTextDelta, readCodexTextDelta, throwIfCodexTerminalEvent } from "../codex-api/responses.js";
...
import { CODEX_REASONING_EFFORTS, COCOPI_REASONING_SUMMARIES, COCOPI_SERVICE_TIERS, codexReasoningFromCocopiOptions, codexServiceTierFromCocopiOptions, codexToolOptionsFromCocopiOptions } from "./configuration.js";
import { logCodexFailurePayloadDiagnostics, logCodexRequestDiagnostics, logCodexResponseEventDiagnostics, logCodexTokenCacheSummary, logCodexWebSocketContinuationDecision, noopCocopiLogger, summarizeCodexRequestBodyForDiagnostics } from "./diagnostics.js";
...
import { codexFunctionCallInputItemFromToolCall, codexToolChoiceFromLanguageModelToolMode, codexToolsFromLanguageModelTools, languageModelQualifiedName, readCodexReasoningInputItem, readCodexToolCall, stableCodexJsonString, stripUnsupportedLanguageModelToolSchemaMetadata, withDefaultRunSubagentToolModel, withOptionalNullToolArgumentsRemoved } from "./tool-bridge.js";
 ...
 * @property {{ registerLanguageModelChatProvider(vendor: string, provider: import("vscode").LanguageModelChatProvider): { dispose(): void } }} lm
  ...
  return {
    ...
    async provideLanguageModelChatResponse(model, messages, requestOptions, progress, token) {
      ...
      try {
        ...
        const showReasoningSummary = shouldShowLanguageModelReasoningSummary(modelOptions, runtime.configuration.reasoningSummary, reasoning?.summary ?? undefined);
        ...
        const body = buildTextResponseBody({
          ...
          ...(requestState.instructions ? { instructions: requestState.instructions } : {}),
        ...
        const continuationAnchors = requestDiagnostics.requestKind === "conversation-summary"
          ? []
          : continuationAnchorsFromLanguageModelRequestState(requestState, body);
        ...
        const reasoningProgress = createLanguageModelReasoningProgressReporter(progress, vscode, showReasoningSummary);
...
function formatLanguageModelCompactionDiagnostic(information, model, options) {
  ...
  const source = compactionDiagnosticSource(information.maxInputTokens, model?.autoCompactTokenLimit, options.useModelDefaultCompactionLimit, fallbackStrategy);
 ...
 */
function compactionDiagnosticSource(maxInputTokens, autoCompactTokenLimit, useModelDefaultCompactionLimit, fallbackStrategy) {
  if (useModelDefaultCompactionLimit !== false && typeof autoCompactTokenLimit === "number" && autoCompactTokenLimit > 0 && maxInputTokens <= autoCompactTokenLimit) {
...
export function languageModelErrorFromCodexError(error, signal, vscode) {
  ...
  if (/status\s+429\b|quota|rate limit|blocked|idle for \d+ms/iu.test(message)) {
    return vscode.LanguageModelError.Blocked("Cocopi request was blocked or timed out.");
  }
...
/**
 * @param {Readonly<Record<string, unknown>> | undefined} modelOptions
 * @param {"default" | "auto" | "off" | "concise" | "detailed"} configuredSummary
 * @param {string | undefined} resolvedSummary
 */
function shouldShowLanguageModelReasoningSummary(modelOptions, configuredSummary, resolvedSummary) {
  const selectedSummary = readRequestOptionForDiagnostics(modelOptions, "reasoningSummary", "reasoning_summary", ["reasoning", "summary"]);
  const selectedVisibility = typeof selectedSummary === "string" ? reasoningSummaryVisibility(selectedSummary) : undefined;
  if (selectedVisibility !== undefined) {
    return selectedVisibility;
  }

  const configuredVisibility = reasoningSummaryVisibility(configuredSummary);
  if (configuredVisibility !== undefined) {
    return configuredVisibility;
  }

  const resolvedVisibility = typeof resolvedSummary === "string" ? reasoningSummaryVisibility(resolvedSummary) : undefined;
  return resolvedVisibility ?? true;
}
 ...
 */
function reasoningSummaryVisibility(summary) {
  switch (summary) {
 ...
 * @param {{ autoCompactTokenLimit?: number | null, useModelDefaultCompactionLimit?: boolean, compactionFallbackStrategy?: "full" | "ninety-percent" }} [options]
 */
function languageModelTokenLimits(contextWindow = DEFAULT_MODEL_CONTEXT_WINDOW, options = {}) {
  const modelContextWindow = contextWindow;
  const maxOutputTokens = Math.min(DEFAULT_MODEL_MAX_OUTPUT_TOKENS, modelContextWindow - 1);
  const fullInputTokens = modelContextWindow - maxOutputTokens;
  let maxInputTokens = fullInputTokens;
  ...
  if (options.useModelDefaultCompactionLimit !== false && typeof options.autoCompactTokenLimit === "number" && options.autoCompactTokenLimit > 0) {
    maxInputTokens = Math.min(options.autoCompactTokenLimit, fullInputTokens);
  } else if (fallbackStrategy === "ninety-percent") {
    maxInputTokens = Math.max(1, Math.floor(fullInputTokens * 0.9));
  }
  return {
    maxInputTokens,
    maxOutputTokens
...
function languageModelInformationFromCodexModel(model, options = {}) {
  ...
  const configurationSchema = cocopiLanguageModelConfigurationSchema(model);
...
/** @param {CodexModelSummary} model */
function cocopiLanguageModelConfigurationSchema(model) {
  const choices = cocopiLanguageModelConfigurationChoices(model);
  if (choices.length < 2) {
    return;
  }

  const defaultEffort = highestReasoningEffort(choices.map((choice) => choice.effort));
  const defaultChoice = defaultEffort
    ? choices.find((choice) => choice.effort === defaultEffort)?.value
    : undefined;
  return {
    properties: {
      [LANGUAGE_MODEL_CONFIGURATION_OPTIONS_KEY]: {
        type: "string",
...
function cocopiLanguageModelConfigurationChoices(model) {
  const efforts = cocopiLanguageModelConfigurationReasoningEfforts(model);
...
function cocopiLanguageModelConfigurationChoice(effort) {
  return {
    effort,
    value: effort,
    label: reasoningEffortLabel(effort),
    description: reasoningOptionDescription(effort)
  };
}

/** @param {CodexModelSummary} model */
function cocopiLanguageModelConfigurationReasoningEfforts(model) {
  if (Array.isArray(model.supportedReasoningLevels)) {
    return uniqueReasoningEfforts(model.supportedReasoningLevels.map((level) => level.effort));
  }

  return model.defaultReasoningLevel ? [...DEFAULT_MODEL_CONFIGURATION_REASONING_EFFORTS] : [];
}
...
function codexModelReasoningOptions(models, modelId) {
  ...
  return {
    defaultEffort: highestReasoningEffort(supportedEfforts) ?? model?.defaultReasoningLevel,
    supportedEfforts,
    supportsSummaries: model?.supportsReasoningSummaries,
...
export function codexRequestStateFromLanguageModelMessages(messages, modelId, vscode, options = {}) {
  ...
  for (const [index, message] of messages.entries()) {
    ...
    const isCompactionSummary = role === "user" && isCompactionSummaryMessage(message);
...
function instructionTextFromLanguageModelMessage(message) {
  if (!message.content.every((part) => part && typeof part === "object" && "value" in part && typeof part.value === "string")) {
    return;
  }
...
function isCompactionSummaryMessage(message) {
  const text = textFromLanguageModelMessage(message).trimStart();
...
function codexOutputTextFromOutputItem(item) {
  ...
  return message ? codexOutputTextFromAssistantMessageInputItem(message) : undefined;
}
...
function decodeCocopiStatefulMarkerPayload(marker, options) {
  ...
  try {
    ...
    const responseItems = rawResponseItems.filter((item) => isCodexMarkerResponseInputItem(item));
 ...
 */
function isCodexMarkerResponseInputItem(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  ...
  if (item.role === "assistant" && Array.isArray(item.content)) {
    const markerContent = /** @type {unknown[]} */ (item.content);
    return markerContent.every((content) => {
      if (!content || typeof content !== "object" || Array.isArray(content)) {
        return false;
      }

      const contentItem = /** @type {Record<string, unknown>} */ (content);
