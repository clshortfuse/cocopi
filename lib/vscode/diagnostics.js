 ...
 * @property {'chat' | 'language-model' | 'inline-completion'} source
 * @property {'chat' | 'utility' | 'autocomplete' | 'unknown'} [workload]
 * @property {'main' | 'general' | 'small' | 'inline' | 'unknown'} [workloadSubtype]
...
export function logCodexTokenCacheSummary(logger, debugLevel, context, options = {}) {
  ...
  if (context.workload) {
    fields.push(`workload=${formatLogScalar(context.workload)}`);
  }
  if (context.workloadSubtype) {
    fields.push(`workloadSubtype=${formatLogScalar(context.workloadSubtype)}`);
  }
  if (context.requestedModel) {
    fields.push(`requestedModel=${formatLogScalar(context.requestedModel)}`);
  }
  if (context.resolvedModel) {
    fields.push(`resolvedModel=${formatLogScalar(context.resolvedModel)}`);
  }
  ...
  if (options.tokenTracking !== false && usage) {
    recordCocopiTokenCacheSummary({
      source: context.source,
      workload: context.workload ?? "unknown",
      workloadSubtype: context.workloadSubtype ?? "unknown",
      requestedModel: context.requestedModel ?? context.selectedModel ?? context.model,
      resolvedModel: context.resolvedModel ?? context.model,
