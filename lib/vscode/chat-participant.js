...
import { createCocopiLogger, logCodexFailurePayloadDiagnostics, logCodexRequestDiagnostics, logCodexResponseEventDiagnostics, logCodexTokenCacheSummary, logCodexWebSocketContinuationDecision, noopCocopiLogger, readCodexUsageSummary, summarizeCodexRequestBodyForDiagnostics } from "./diagnostics.js";
...
async function streamToolFollowUp(options) {
  ...
  while (pendingToolCalls.length > 0) {
    ...
    for (const toolCall of pendingToolCalls) {
      ...
      options.response.progress(`Running ${toolCall.name}.`);
      const result = await options.vscode.lm.invokeTool(toolCall.name, {
