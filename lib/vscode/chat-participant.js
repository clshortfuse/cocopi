...
export function createCocopiChatRequestHandler(context, vscode, options = {}) {
  ...
  return async (request, chatContext, response, token) => {
    ...
    try {
      ...
      const reasoningProgress = createChatReasoningProgressReporter(response, vscode);
...
async function streamToolFollowUp(options) {
  ...
  while (pendingToolCalls.length > 0) {
    ...
    /** @type {import("../../data/Codex.js").CodexResponseCompletedEvent["response"] | undefined} */
    let completedResponse;
    const reasoningProgress = createChatReasoningProgressReporter(options.response, options.vscode);
    let lastTextDeltaOutputIndex;
    try {
 ...
 */
function createChatReasoningProgressReporter(response, vscode) {
  /** @type {string | undefined} */
  let sanitizedPartId;
  const sanitizer = createReasoningSummaryHtmlCommentSanitizer(
     ...
     */
    (text, options) => reportChatReasoningProgress(response, vscode, text, options)
  );
