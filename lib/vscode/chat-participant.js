...
import { codexReasoningSummaryMetadata, codexReasoningSummaryPartId, readCodexReasoningSummaryTextDelta, readCodexTextDelta, throwIfCodexTerminalEvent } from "../codex-api/responses.js";
...
import { COCOPI_CHAT_PARTICIPANT_MODEL_SOURCES, COCOPI_SERVICE_TIERS, codexReasoningFromCocopiOptions, codexToolOptionsFromCocopiOptions, cocopiUltraMultiAgentModeFromOptions, resolveChatParticipantInstructions, resolveCocopiUltraMultiAgentInstructions, resolveVscodeLanguageModelTools } from "./configuration.js";
...
import { createReasoningSummaryHtmlCommentSanitizer } from "./visible-text.js";
...
export function createCocopiChatRequestHandler(context, vscode, options = {}) {
  ...
  return async (request, chatContext, response, token) => {
    ...
    try {
      ...
      const requestLanguageModelTools = resolveVscodeLanguageModelTools(userTools, requestRuntime.configuration);
      const requestTools = codexToolsFromLanguageModelTools(requestLanguageModelTools, toolOptions);
      const explicitlyRequestedToolNames = new Set(request.toolReferences.map((toolReference) => toolReference.name));
      const explicitlyRequestedToolAvailable = userTools.some((tool) => explicitlyRequestedToolNames.has(tool.name));
      const runSubagentDefaultModel = chatRequestLanguageModelQualifiedName(request, requestModel);
      const ultraMultiAgentMode = cocopiUltraMultiAgentModeFromOptions(requestRuntime.configuration, undefined, requestLanguageModelTools);
      requestInstructions = resolveCocopiUltraMultiAgentInstructions(
        resolveChatParticipantInstructions(COCOPI_CHAT_PARTICIPANT_INSTRUCTIONS, requestRuntime.configuration),
        ultraMultiAgentMode
      );
      ...
      const body = buildTextResponseBody({
        ...
        input,
        tools: requestTools,
        toolChoice: explicitlyRequestedToolAvailable ? "required" : "auto",
        parallelToolCalls: ultraMultiAgentMode,
        stream: true,
      ...
      const reasoningProgress = createChatReasoningProgressReporter(response, vscode, logger);
      ...
      try {
        for await (const event of events) {
          ...
          if (reasoningSummaryDelta) {
            markFirstOutput();
            reasoningProgress.report(reasoningSummaryDelta, {
...
async function streamToolFollowUp(options) {
  ...
  const ultraMultiAgentMode = cocopiUltraMultiAgentModeFromOptions(options.runtime.configuration, undefined, options.userTools);
  const followUpInstructions = resolveCocopiUltraMultiAgentInstructions(
    resolveChatParticipantInstructions(COCOPI_CHAT_PARTICIPANT_INSTRUCTIONS, options.runtime.configuration),
    ultraMultiAgentMode
  );
  const runSubagentDefaultModel = chatRequestLanguageModelQualifiedName(options.request, options.model);
  let visibleResponseText = options.visibleResponseText;
  ...
  while (pendingToolCalls.length > 0) {
    ...
    const toolResults = await Promise.all(pendingToolCalls.map(async (toolCall) => {
      throwIfChatCancellationRequested(options);
    ...
    for (const [index, toolCall] of pendingToolCalls.entries()) {
      followUpInput.push(
        codexFunctionCallInputItemFromToolCall(toolCall),
        codexFunctionCallOutputInputItemFromToolResult(toolCall.callId, toolResults[index])
      );
    }

    if (pendingToolCalls.length === 1 && pendingToolCalls[0].name === "task_complete") {
      const summary = taskCompleteSummaryFromInput(pendingToolCalls[0].input);
      const alreadyVisible = summary ? visibleResponseText.includes(summary) : Boolean(visibleResponseText.trim());
      if (summary && !alreadyVisible) {
        options.response.markdown(summary);
      }
      if (summary || alreadyVisible) {
        options.logger.info(`Chat completed after terminal task_complete result. source=chat summaryPresent=${Boolean(summary)} alreadyVisible=${alreadyVisible}`);
        return followUpInput.slice(options.input.length);
      }
      options.logger.info("Chat terminal task_complete result had no visible summary; requesting a model follow-up. source=chat");
    }
    ...
    /** @type {import("../../data/Codex.js").CodexResponseCompletedEvent["response"] | undefined} */
    let completedResponse;
    const reasoningProgress = createChatReasoningProgressReporter(options.response, options.vscode, options.logger);
    let lastTextDeltaOutputIndex;
    try {
...
function taskCompleteSummaryFromInput(input) {
 ...
 */
function createChatReasoningProgressReporter(response, vscode, logger) {
  /** @type {string | undefined} */
  let sanitizedPartId;
  /** @type {{ new(value: string | string[], id?: string, metadata?: Record<string, unknown>): unknown } | undefined} */
  let NativeThinkingPart;
  try {
    NativeThinkingPart = vscode.ChatResponseThinkingProgressPart;
  } catch (error) {
    logger.info(`Cocopi native Chat thinking parts are unavailable; using markdown fallback. error=${error instanceof Error ? error.message : String(error)}`);
  }
  const sanitizer = createReasoningSummaryHtmlCommentSanitizer(
    /**
     * @param {string} text
    ...
    (text, options) => {
      ...
      if (NativeThinkingPart) {
        try {
          response.push(/** @type {import("vscode").ChatResponsePart} */ (new NativeThinkingPart(text, options.id, options.metadata)));
          return;
        } catch (error) {
...
function languageModelToolsFromChatRequest(request, vscode) {
  ...
  requestedNames.add("runSubagent");
