...
export function codexToolsFromLanguageModelTools(tools, options = {}) {
  ...
  return [...tools]
    ...
    .map(({ tool }) => ({
      ...
      parameters: codexToolParametersFromInputSchema(tool.inputSchema, options),
...
/**
 * @param {CodexResponseStreamEvent} event
 * @returns {{ callId: string, name: string, outputIndex: number, sequenceNumber?: number } | undefined}
 */
