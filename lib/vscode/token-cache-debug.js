...
const MAX_COCOPI_TOKEN_CACHE_VERBOSE_ENTRIES = 100;
...
const COCOPI_TOKEN_CACHE_DEBUG_STORAGE_VERSION = 2;
...
/** @typedef {'chat' | 'language-model' | 'inline-completion'} CocopiTokenCacheDebugSource */
/** @typedef {'chat' | 'utility' | 'autocomplete' | 'unknown'} CocopiWorkload */
/** @typedef {'main' | 'general' | 'small' | 'inline' | 'unknown'} CocopiWorkloadSubtype */
...
/** @typedef {Omit<CocopiTokenCacheDebugSummary, "id" | "recordedAt" | "workload" | "workloadSubtype" | "requestedModel" | "resolvedModel"> & Partial<Pick<CocopiTokenCacheDebugSummary, "workload" | "workloadSubtype" | "requestedModel" | "resolvedModel">>} CocopiTokenCacheDebugSummaryInput */

/**
 * Persist only fields used by token/cache analytics and the tracker UI. Raw
 * request identifiers and verbose mismatch descriptions remain available on
 * recent live rows, while payload diagnostics retain the full request data.
 *
 * @type {(keyof CocopiTokenCacheDebugSummary)[]}
 */
const COCOPI_TOKEN_CACHE_PERSISTED_FIELDS = [
  "id",
  "recordedAt",
  "source",
  "hostRequestIndex",
  "lastHostRequestIndex",
  "mergedRequestCount",
  "sessionId",
  "conversationSummary",
  ...
  "requestMessages",
  "requestTextParts",
  "requestToolCallParts",
  "requestToolResultParts",
  "requestDataParts",
  "requestCocopiDataParts",
  "requestCocopiDataBytes",
  ...
  "webSocketContinuationMatchingItems",
  "webSocketContinuationMismatchIndex",
  "webSocketContinuationExpectedDigest",
  "webSocketContinuationActualDigest",
  "turnKind",
  "cacheRisk",
  "uncachedInputTokens",
  "requestDurationMs",
...
/** @type {Set<keyof CocopiTokenCacheDebugSummary>} */
const COCOPI_TOKEN_CACHE_PERSISTED_STRING_FIELDS = new Set([
  "source",
  "workload",
  "workloadSubtype",
  "requestedModel",
 ...
 * @property {number} outputTokens
 * @property {number} uncachedInputTokens
 * @property {Array<{ workloadSubtype: CocopiWorkloadSubtype, label: string, requestCount: number, billableTokens: number, inputTokens: number, outputTokens: number, uncachedInputTokens: number }>} subtypes
 */
...
/**
 * @typedef {object} CocopiTokenCacheDebugStorageState
 * @property {import("./secret-storage.js").SecretStorageLike} storage
 * @property {Promise<void>} load
 * @property {Promise<void> | undefined} drain
 * @property {boolean} dirty
 */

/** @type {CocopiTokenCacheDebugStorageState | undefined} */
let cocopiTokenCacheDebugStorageState;
...
export function initializeCocopiTokenCacheDebugStorage(secrets) {
  ...
  const state = /** @type {CocopiTokenCacheDebugStorageState} */ ({
    storage: secrets,
    load: Promise.resolve(),
    drain: undefined,
    dirty: false
  });
  cocopiTokenCacheDebugStorageState = state;
  state.load = loadCocopiTokenCacheDebugSummariesFromStorage(secrets).then((requiresMigration) => {
    if (requiresMigration && cocopiTokenCacheDebugStorageState === state) {
      requestCocopiTokenCacheDebugPersistence(state);
    }
  });
  return state.load;
}
...
/** @returns {Promise<void>} */
export async function waitForCocopiTokenCacheDebugStorage() {
  const state = cocopiTokenCacheDebugStorageState;
  if (!state) {
    return;
  }
...
/** @param {CocopiTokenCacheDebugSummaryInput} summary */
export function recordCocopiTokenCacheSummary(summary) {
  ...
  const entry = /** @type {CocopiTokenCacheDebugSummary} */ ({
    id: nextCocopiTokenCacheDebugId++,
    recordedAt: new Date().toISOString(),
    source: summary.source,
    workload: normalizeWorkload(summary.workload),
    workloadSubtype: normalizeWorkloadSubtype(summary.workloadSubtype),
    requestedModel: summary.requestedModel ?? summary.selectedModel ?? summary.model,
    resolvedModel: summary.resolvedModel ?? summary.model,
  ...
  if (cocopiTokenCacheDebugHistory.length > MAX_COCOPI_TOKEN_CACHE_VERBOSE_ENTRIES) {
    cocopiTokenCacheDebugHistory[MAX_COCOPI_TOKEN_CACHE_VERBOSE_ENTRIES] = durableCocopiTokenCacheDebugSummary(
      cocopiTokenCacheDebugHistory[MAX_COCOPI_TOKEN_CACHE_VERBOSE_ENTRIES]
    );
  }
...
/**
 * @param {{ now?: Date, timelineDays?: number, workload?: CocopiWorkload, workloadSubtype?: CocopiWorkloadSubtype }} [options]
 * @returns {CocopiUsageAnalytics}
 */
export function readCocopiUsageAnalytics(options = {}) {
  const now = options.now ?? new Date();
  const entries = filteredTokenCacheEntries(options.workload, options.workloadSubtype);
  const windows = COCOPI_USAGE_ANALYTICS_WINDOW_HOURS.map((windowHours) => usageAnalyticsWindow(windowHours, now, entries));
  return {
    capturedAt: now.toISOString(),
    windows,
    weeklyCycle: weeklyCycleUsage(now, entries),
    timeline: usageTimeline(now, options.timelineDays, entries),
    rateLimitTrends: rateLimitTrends(now, Math.max(...COCOPI_USAGE_ANALYTICS_WINDOW_HOURS)),
    sessions: sessionUsageSummaries(entries),
    workloads: workloadUsageSummaries(entries),
    remoteUsageAnalytics: readCocopiRemoteUsageAnalyticsSnapshots()[0],
    retainedRequestRows: cocopiTokenCacheDebugHistory.length,
    matchingRequestRows: entries.length,
...
async function loadCocopiTokenCacheDebugSummariesFromStorage(secrets) {
  ...
  cocopiTokenCacheDebugHistory.length = 0;
  cocopiTokenCacheDebugHistory.push(...mergeStoredCocopiTokenCacheDebugSummaries(current, parsed.entries)
    .slice(0, MAX_COCOPI_TOKEN_CACHE_DEBUG_ENTRIES)
    .map((entry, index) => index < MAX_COCOPI_TOKEN_CACHE_VERBOSE_ENTRIES ? entry : durableCocopiTokenCacheDebugSummary(entry)));
  nextCocopiTokenCacheDebugId = Math.max(0, ...cocopiTokenCacheDebugHistory.map((entry) => entry.id)) + 1;
  return parsed.requiresMigration;
}
...
/**
 * @param {string} stored
 * @returns {{ entries: CocopiTokenCacheDebugSummary[], requiresMigration: boolean }}
 */
function parseStoredTokenCacheDebugSummaries(stored) {
  try {
    ...
    if (Array.isArray(parsed)) {
      return {
        entries: parsed.map((summary) => sanitizeStoredTokenCacheDebugSummary(summary)).filter((summary) => summary !== undefined),
        requiresMigration: true
      };
    }

    if (!parsed || typeof parsed !== "object") {
      return { entries: [], requiresMigration: false };
    }

    const payload = /** @type {Record<string, unknown>} */ (parsed);
    if (payload.version !== COCOPI_TOKEN_CACHE_DEBUG_STORAGE_VERSION || !Array.isArray(payload.strings) || !Array.isArray(payload.rows)) {
      return { entries: [], requiresMigration: false };
    }

    const strings = /** @type {unknown[]} */ (payload.strings);
    return {
      entries: payload.rows.map((row) => decodePersistedCocopiTokenCacheDebugSummary(row, strings)).filter((summary) => summary !== undefined),
 ...
 * @returns {CocopiTokenCacheDebugSummary}
 */
function durableCocopiTokenCacheDebugSummary(entry) {
  const source = /** @type {Record<string, unknown>} */ (entry);
  /** @type {Record<string, unknown>} */
  const result = {};
  for (const field of COCOPI_TOKEN_CACHE_PERSISTED_FIELDS) {
    const value = source[field];
    if (value !== undefined) {
      result[field] = value;
    }
  }
  return /** @type {CocopiTokenCacheDebugSummary} */ (result);
}

/** @returns {string} */
function serializeCocopiTokenCacheDebugSummaries() {
  /** @type {string[]} */
  const strings = [];
  /** @type {Map<string, number>} */
  const stringIndexes = new Map();
  const rows = cocopiTokenCacheDebugHistory.map((entry) => {
    const source = /** @type {Record<string, unknown>} */ (entry);
    /** @type {(number | boolean | null)[]} */
    const row = COCOPI_TOKEN_CACHE_PERSISTED_FIELDS.map((field) => {
      const value = source[field];
      if (value === undefined) {
        return null;
      }
      ...
      if (COCOPI_TOKEN_CACHE_PERSISTED_STRING_FIELDS.has(field)) {
  ...
  return JSON.stringify({
    version: COCOPI_TOKEN_CACHE_DEBUG_STORAGE_VERSION,
    strings,
    rows
  });
}
 ...
 * @returns {CocopiTokenCacheDebugSummary | undefined}
 */
function decodePersistedCocopiTokenCacheDebugSummary(value, strings) {
  ...
  /** @type {Record<string, unknown>} */
  const result = {};
  for (let index = 0; index < Math.min(value.length, COCOPI_TOKEN_CACHE_PERSISTED_FIELDS.length); index += 1) {
    const field = COCOPI_TOKEN_CACHE_PERSISTED_FIELDS[index];
    ...
    if (field === "recordedAt") {
      if (typeof stored === "number" && Number.isFinite(stored)) {
        result[field] = new Date(stored).toISOString();
      }
      continue;
    }
    if (COCOPI_TOKEN_CACHE_PERSISTED_STRING_FIELDS.has(field)) {
      if (typeof stored === "number" && Number.isInteger(stored) && typeof strings[stored] === "string") {
        result[field] = strings[stored];
      }
      continue;
    }
  ...
  return sanitizeStoredTokenCacheDebugSummary(result);
}
...
function sanitizeStoredTokenCacheDebugSummary(value) {
  ...
  const source = entry.source === "chat" || entry.source === "language-model" || entry.source === "inline-completion" ? entry.source : undefined;
  ...
  const result = /** @type {CocopiTokenCacheDebugSummary} */ ({
    ...
    recordedAt,
    source,
    workload: normalizeWorkload(readOptionalString(entry.workload)),
    workloadSubtype: normalizeWorkloadSubtype(readOptionalString(entry.workloadSubtype)),
    requestedModel: readOptionalString(entry.requestedModel) ?? readOptionalString(entry.selectedModel) ?? model,
    resolvedModel: readOptionalString(entry.resolvedModel) ?? model,
    hostRequestIndex,
...
/** @param {CocopiTokenCacheDebugStorageState} state */
function requestCocopiTokenCacheDebugPersistence(state) {
  state.dirty = true;
  if (state.drain || cocopiTokenCacheDebugStorageState !== state) {
    return;
  }

  const drain = drainCocopiTokenCacheDebugPersistence(state);
  state.drain = drain.finally(() => {
    state.drain = undefined;
    if (state.dirty && cocopiTokenCacheDebugStorageState === state) {
      requestCocopiTokenCacheDebugPersistence(state);
    }
  });
  void state.drain.catch(() => {});
}

/** @param {CocopiTokenCacheDebugStorageState} state */
async function drainCocopiTokenCacheDebugPersistence(state) {
  await state.load;
  while (state.dirty && cocopiTokenCacheDebugStorageState === state) {
    state.dirty = false;
    await state.storage.store(COCOPI_TOKEN_CACHE_DEBUG_STORAGE_KEY, serializeCocopiTokenCacheDebugSummaries());
  }
}
...
/**
 * @param {Date} now
 * @param {CocopiTokenCacheDebugSummary[]} entriesSource
 */
function weeklyCycleUsage(now, entriesSource) {
  ...
  const entries = entriesWithinWindow(cycleStartMs, aggregateEndMs, entriesSource);
 ...
 */
function usageTimeline(now, requestedDays, entriesSource) {
  ...
  for (const entry of entriesWithinWindow(windowStartMs, windowEndMs, entriesSource)) {
    const recordedAtMs = Date.parse(entry.recordedAt);
 ...
 */
function usageAnalyticsWindow(windowHours, now, entriesSource) {
  const windowEndMs = now.getTime();
  const windowStartMs = windowEndMs - windowHours * 60 * 60 * 1000;
  const totals = aggregateTokenCacheEntries(entriesWithinWindow(windowStartMs, windowEndMs, entriesSource));
 ...
 * @param {CocopiTokenCacheDebugSummary[]} [entriesSource]
 */
function entriesWithinWindow(windowStartMs, windowEndMs, entriesSource = cocopiTokenCacheDebugHistory) {
 ...
 */
function sessionUsageSummaries(entriesSource) {
  /** @type {Map<string, CocopiTokenCacheDebugSummary[]>} */
  const groups = new Map();
...
/**
 * @param {CocopiWorkload | undefined} workload
 * @param {CocopiWorkloadSubtype | undefined} workloadSubtype
 */
function filteredTokenCacheEntries(workload, workloadSubtype) {
  return cocopiTokenCacheDebugHistory.filter((entry) => (
    (!workload || entry.workload === workload)
    && (!workloadSubtype || entry.workloadSubtype === workloadSubtype)
  ));
}

/**
 * @param {CocopiTokenCacheDebugSummary[]} entries
 * @returns {CocopiWorkloadUsageSummary[]}
 */
function workloadUsageSummaries(entries) {
  /** @type {Map<CocopiWorkload, CocopiTokenCacheDebugSummary[]>} */
  const groups = new Map();
  for (const entry of entries) {
    groups.set(entry.workload, [...(groups.get(entry.workload) ?? []), entry]);
  }

  return [...groups.entries()].map(([workload, workloadEntries]) => {
    const totals = aggregateTokenCacheEntries(workloadEntries);
    /** @type {Map<CocopiWorkloadSubtype, CocopiTokenCacheDebugSummary[]>} */
    const subtypeGroups = new Map();
    for (const entry of workloadEntries) {
      subtypeGroups.set(entry.workloadSubtype, [...(subtypeGroups.get(entry.workloadSubtype) ?? []), entry]);
    }
    return {
      workload,
      ...
      outputTokens: totals.outputTokens,
      uncachedInputTokens: totals.uncachedInputTokens,
      subtypes: [...subtypeGroups.entries()].map(([workloadSubtype, subtypeEntries]) => {
        const subtypeTotals = aggregateTokenCacheEntries(subtypeEntries);
        return {
          workloadSubtype,
          label: workloadSubtypeLabel(workloadSubtype),
          requestCount: subtypeTotals.requestCount,
          billableTokens: subtypeTotals.billableTokens,
          inputTokens: subtypeTotals.inputTokens,
          outputTokens: subtypeTotals.outputTokens,
          uncachedInputTokens: subtypeTotals.uncachedInputTokens
        };
      }).toSorted((left, right) => right.billableTokens - left.billableTokens || left.label.localeCompare(right.label))
    };
  }).toSorted((left, right) => workloadRank(left.workload) - workloadRank(right.workload));
}

/**
 * @param {string | undefined} value
 * @returns {CocopiWorkload}
 */
function normalizeWorkload(value) {
  return value === "chat" || value === "utility" || value === "autocomplete" ? value : "unknown";
}

/**
 * @param {string | undefined} value
 * @returns {CocopiWorkloadSubtype}
 */
function normalizeWorkloadSubtype(value) {
  return value === "main" || value === "general" || value === "small" || value === "inline" ? value : "unknown";
}

/** @param {CocopiWorkload} workload */
function workloadLabel(workload) {
  ...
  if (workload === "autocomplete") {
    return "Autocomplete";
  }
  return "Unknown";
}

/** @param {CocopiWorkloadSubtype} workloadSubtype */
function workloadSubtypeLabel(workloadSubtype) {
  if (workloadSubtype === "main") {
    return "Main";
  }
  if (workloadSubtype === "general") {
    return "General";
  }
  if (workloadSubtype === "small") {
    return "Small";
  }
  if (workloadSubtype === "inline") {
    return "Inline";
  }
  return "Unknown";
}
...
function workloadRank(workload) {
  return ["chat", "utility", "autocomplete", "unknown"].indexOf(workload);
}
