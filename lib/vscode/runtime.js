...
/**
 * Runtime auth intentionally excludes refresh and ID tokens. Those are only
 * materialized long enough to refresh or derive metadata from SecretStorage.
 *
 * @typedef {Pick<StoredCodexAuth, "accessToken" | "chatgptAccountId" | "chatgptPlanType">} CocopiRuntimeAuth
 */
 ...
 * @property {CocopiRuntimeAuth | undefined} auth
...
export async function readCocopiRuntime(context, vscode, options = {}) {
  const auth = options.refreshAuth === false ? runtimeAuthFromStoredAuth(await readCodexAuth(context.secrets)) : await readFreshCodexAuth(context);
  return {
...
/**
 * @param {CocopiSecretContext} context
 * @returns {Promise<CocopiRuntimeAuth | undefined>}
 */
...
async function readFreshCodexAuth(context) {
  ...
  if (!auth || !isJwtExpired(auth.accessToken)) {
    return runtimeAuthFromStoredAuth(auth);
  }
...
async function refreshStoredAuth(context, auth) {
  ...
  return runtimeAuthFromStoredAuth(refreshedAuth);
}

/**
 * @param {StoredCodexAuth | undefined} auth
 * @returns {CocopiRuntimeAuth | undefined}
 */
function runtimeAuthFromStoredAuth(auth) {
  if (!auth) {
    return;
  }
