...
/** @param {Error} error */
function isRetryableEarlyWebSocketClose(error) {
  return error instanceof CodexResponseWebSocketError
    && /closed before a terminal response event/u.test(error.message)
    && /code=1000/u.test(error.message);
}
...
function isRetryableWebSocketFailureBeforeOutput(error, body) {
  if (isRetryableEarlyWebSocketClose(error)) {
    return true;
  }
