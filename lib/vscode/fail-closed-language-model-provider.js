import { COCOPI_WORKLOAD_ALIASES, isCocopiWorkloadAlias } from "./workload-routes.js";
...
function cocopiConfiguredLanguageModelIds(vscode) {
  const modelIds = new Set(Object.values(COCOPI_WORKLOAD_ALIASES).map((alias) => alias.slice(`${COCOPI_LANGUAGE_MODEL_VENDOR}/`.length)));
...
function genericLanguageModelInformation(model, detail, name = model) {
  return {
    ...
    version: model,
    isBYOK: true,
    isUserSelectable: !isCocopiWorkloadAlias(model),
