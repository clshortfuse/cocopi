...
/**
 * Keep Cocopi model IDs resolvable and consume every Cocopi-owned request locally.
 * If model discovery rejects or forgets an ID, VS Code can silently substitute a
 * Copilot model before the selected provider receives the request.
 *
 * @param {FailClosedContext} context
 * @param {FailClosedVscodeApi} vscode
 * @param {{ logger?: FailClosedLogger }} [options]
  ...
  const provider = {
    ...
    async provideLanguageModelChatResponse(model, messages, requestOptions, progress, token) {
      ...
      try {
        ...
        logFailClosedError(logger, "Cocopi request failed at the fail-closed provider boundary; no replacement provider was invoked.", normalizeCaughtError(error));
        if (!token.isCancellationRequested) {
          reportLocalCocopiFailure(progress, vscode, model.id, "The Cocopi request failed. It might have incurred usage before the failure.", logger);
        }
      }
    },
