...
import { recordCocopiInstructionReplacementSnapshot } from "./instruction-replacements.js";
import { COCOPI_WORKLOAD_ROUTE_DEFAULTS } from "./workload-routes.js";
...
/**
 * @typedef {object} CocopiWorkloadRouteConfiguration
 * @property {string} model
 * @property {"lowest" | "default" | "none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"} reasoningEffort
 ...
 * @property {{ utility: CocopiWorkloadRouteConfiguration, utilitySmall: CocopiWorkloadRouteConfiguration, autocomplete: CocopiWorkloadRouteConfiguration }} routes
...
export function readCocopiConfiguration(vscode) {
  ...
  return {
    ...
    routes: {
      utility: readWorkloadRouteConfiguration(configuration, "routes.utility", COCOPI_WORKLOAD_ROUTE_DEFAULTS.utility),
      utilitySmall: readWorkloadRouteConfiguration(configuration, "routes.utilitySmall", COCOPI_WORKLOAD_ROUTE_DEFAULTS.utilitySmall),
      autocomplete: readWorkloadRouteConfiguration(configuration, "routes.autocomplete", COCOPI_WORKLOAD_ROUTE_DEFAULTS.autocomplete)
    },
    inlineCompletions: {
 ...
 * @param {string} prefix
 * @param {{ model: string, reasoningEffort: string, serviceTier: string }} defaults
 * @returns {CocopiWorkloadRouteConfiguration}
 */
function readWorkloadRouteConfiguration(configuration, prefix, defaults) {
  return {
    model: String(configuration.get(`${prefix}.model`, defaults.model)).trim(),
    reasoningEffort: normalizeWorkloadReasoningEffort(configuration.get(`${prefix}.reasoningEffort`, defaults.reasoningEffort), defaults.reasoningEffort),
    serviceTier: normalizeServiceTier(configuration.get(`${prefix}.serviceTier`, defaults.serviceTier))
  };
}
...
export function resolveVscodeLanguageModelTools(tools, configuration) {
  ...
  const rewrittenTools = tools.map((tool) => {
    ...
    const description = replaceTextWithRegexMap(
      tool.description,
      replacements,
  ...
  const previewEntries = toolDescriptionReplacementEntries(tools, rewrittenTools);
  const sourcePreview = toolDescriptionReplacementPreview(previewEntries, "original");
  if (sourcePreview !== undefined) {
    const rewrittenPreview = toolDescriptionReplacementPreview(previewEntries, "rewritten") ?? sourcePreview;
    recordCocopiInstructionReplacementSnapshot(
      "toolDescriptions",
      sourcePreview,
      rewrittenPreview,
...
/**
 * @param {readonly { name: string, description?: string }[]} tools
 * @param {readonly { name: string, description?: string }[]} rewrittenTools
 */
function toolDescriptionReplacementEntries(tools, rewrittenTools) {
  return tools.flatMap((tool, index) => typeof tool.description === "string" ? [{
    label: tool.name,
    original: tool.description,
    rewritten: rewrittenTools[index]?.description ?? tool.description
  }] : []).toSorted((left, right) => left.label.localeCompare(right.label, undefined, { sensitivity: "base" }));
}

/**
 * @param {{ original: string, rewritten: string }[]} entries
 * @param {"original" | "rewritten"} field
 */
function toolDescriptionReplacementPreview(entries, field) {
  return entries.length > 0 ? entries.map((entry) => entry[field]).join("\n\n---\n\n") : undefined;
}
...
function resolveCocopiInstructions(sourceInstructions, configuration) {
  const source = typeof sourceInstructions === "string" ? sourceInstructions.trim() : "";
  ...
  recordCocopiInstructionReplacementSnapshot("instructions", source, baseInstructions);
 ...
 * @returns {CocopiWorkloadRouteConfiguration["reasoningEffort"]}
 */
function normalizeWorkloadReasoningEffort(value, defaultValue) {
  switch (value) {
    case "lowest":
    case COCOPI_REASONING_EFFORTS.default:
    case COCOPI_REASONING_EFFORTS.none:
    case COCOPI_REASONING_EFFORTS.minimal:
    case COCOPI_REASONING_EFFORTS.low:
    case COCOPI_REASONING_EFFORTS.medium:
    ...
    default: {
      return /** @type {CocopiWorkloadRouteConfiguration["reasoningEffort"]} */ (defaultValue);
    }
  }
}
...
function normalizeReasoningEffortOption(value, supportedEfforts) {
  if (value === COCOPI_REASONING_EFFORTS.default || value === COCOPI_REASONING_EFFORTS.ultra || isKnownCodexReasoningEffort(value)) {
    return value;
  }
...
function resolveCodexReasoningEffort(effort, options) {
  ...
  // Upstream translates the symbolic Ultra selection at the request boundary
  // regardless of whether proactive multi-agent instructions are available.
 ...
 */
function replaceTextWithRegexMap(text, replacements, flags, defaults) {
  let rewritten = text;
  for (const [pattern, replacement] of Object.entries(replacements)) {
    if (!pattern.trim() || (replacement === "" && Object.hasOwn(defaults, pattern))) {
      continue;
    }
