...
export const COCOPI_CHAT_INSTRUCTIONS_MODES = Object.freeze({
  optional: "optional",
  replace: "replace",
  append: "append",
  regex: "regex"
 ...
 * @property {"default" | "none" | "minimal" | "low" | "medium" | "high" | "xhigh"} reasoningEffort
 * @property {"default" | "auto" | "off" | "concise" | "detailed"} reasoningSummary
 ...
 * @property {"optional" | "replace" | "append" | "regex"} chatInstructionsMode
 * @property {string} chatInstructionsRegexPattern
 * @property {string} chatInstructionsRegexReplacement
 * @property {string} chatInstructionsRegexFlags
...
export function readCocopiConfiguration(vscode) {
  ...
  return {
    ...
    reasoningSummary: normalizeReasoningSummary(configuration.get("reasoningSummary", COCOPI_REASONING_SUMMARIES.default)),
    ...
    chatInstructions,
    chatInstructionsMode: normalizeChatInstructionsMode(configuration.get("chatInstructionsMode", COCOPI_CHAT_INSTRUCTIONS_MODES.optional)),
    chatInstructionsRegexPattern: String(configuration.get("chatInstructionsRegexPattern", "")),
    chatInstructionsRegexReplacement: String(configuration.get("chatInstructionsRegexReplacement", "")),
    chatInstructionsRegexFlags: String(configuration.get("chatInstructionsRegexFlags", "g")),
    streamIdleTimeoutMs,
...
export function resolveChatParticipantInstructions(sourceInstructions, configuration) {
  const baseInstructions = typeof sourceInstructions === "string" ? sourceInstructions.trim() : "";
  const configuredInstructions = configuration.chatInstructions.trim();

  if (!baseInstructions && configuration.chatInstructionsMode !== COCOPI_CHAT_INSTRUCTIONS_MODES.replace) {
    return;
  }
  if (!baseInstructions && configuration.chatInstructionsMode === COCOPI_CHAT_INSTRUCTIONS_MODES.replace) {
  ...
  switch (configuration.chatInstructionsMode) {
    case COCOPI_CHAT_INSTRUCTIONS_MODES.optional: {
      return;
    }
    case COCOPI_CHAT_INSTRUCTIONS_MODES.append: {
      return configuredInstructions ? `${baseInstructions}\n\n${configuredInstructions}` : baseInstructions;
    }
    case COCOPI_CHAT_INSTRUCTIONS_MODES.regex: {
      const pattern = configuration.chatInstructionsRegexPattern.trim();
      if (!pattern) {
        return baseInstructions;
      }
      try {
        const regex = new RegExp(pattern, configuration.chatInstructionsRegexFlags);
        return baseInstructions.replace(regex, configuration.chatInstructionsRegexReplacement);
      } catch {
        return baseInstructions;
      }
    }
...
export function codexReasoningFromCocopiOptions(configuration, modelOptions, options = {}) {
  const configuredEffort = normalizeReasoningEffort(readStringModelOption(modelOptions, "reasoningEffort", "reasoning_effort", ["reasoning", "effort"]) ?? configuration.reasoningEffort);
...
/**
 * @param {string} value
 * @returns {"default" | "none" | "minimal" | "low" | "medium" | "high" | "xhigh"}
 */
...
function resolveCodexReasoningEffort(effort, options) {
  const resolved = effort === COCOPI_REASONING_EFFORTS.default ? options.defaultEffort : effort;
  if (!resolved) {
    return;
  }

  const supported = options.supportedEfforts?.filter((value) => CODEX_REASONING_EFFORTS.includes(value));
  ...
  return nearestReasoningEffort(resolved, supported);
}
...
/**
 * @param {string} value
 * @returns {"default" | "auto" | "off" | "concise" | "detailed"}
 */
function normalizeReasoningSummary(value) {
  switch (value) {
    case COCOPI_REASONING_SUMMARIES.default:
    ...
    default: {
      return COCOPI_REASONING_SUMMARIES.default;
    }
  }
}

/**
 * @param {"default" | "auto" | "off" | "concise" | "detailed"} summary
...
function resolveCodexReasoningSummary(summary, options) {
  ...
  if (summary === COCOPI_REASONING_SUMMARIES.default) {
...
/**
 * @param {string} value
 * @returns {"optional" | "replace" | "append" | "regex"}
 */
function normalizeChatInstructionsMode(value) {
  switch (value) {
    case COCOPI_CHAT_INSTRUCTIONS_MODES.optional:
    case COCOPI_CHAT_INSTRUCTIONS_MODES.replace:
    case COCOPI_CHAT_INSTRUCTIONS_MODES.append:
    case COCOPI_CHAT_INSTRUCTIONS_MODES.regex: {
      return value;
    }
    default: {
      return COCOPI_CHAT_INSTRUCTIONS_MODES.optional;
    }
  }
}
