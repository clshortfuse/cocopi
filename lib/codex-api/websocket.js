...
export class CodexResponseWebSocketError extends CodexResponseStreamError {
   ...
   * @param {{ event?: CodexResponseStreamEvent | Record<string, CodexJsonValue>, eventData?: string, cause?: Error }} [options]
   */
...
export function isCodexPreviousResponseNotFoundError(error) {
  return codexWebSocketErrorCode(error) === "previous_response_not_found"
...
function websocketErrorFromEvent(event) {
  ...
  const code = stringFromJsonValue(error?.code);
  ...
  return new CodexResponseWebSocketError(parts.join("; "), { event });
}
...
function codexWebSocketErrorCode(error) {
  const event = recordFromJsonValue(Reflect.get(error, "event"));
  const eventError = recordFromJsonValue(event?.error);
  return stringFromJsonValue(eventError?.code) ?? /\bcode=([a-z0-9_-]+)/iu.exec(error.message)?.[1];
}
